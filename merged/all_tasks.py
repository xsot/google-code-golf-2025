...
# task 133: 290 (390 unzipped) vs 298 bytes for gold, https://arcprize.org/play?task=57aa92db
def p(g):
...
# task 145: 194 vs 191 bytes for gold, https://arcprize.org/play?task=6455b5f5
...
# task 154: 114 vs 99 bytes for gold, https://arcprize.org/play?task=6855a6e4
...
# task 157: 257 (271 unzipped) vs 248 bytes for gold, https://arcprize.org/play?task=6a1e5592
def p(g):
 ...
 for B,r in enumerate(g):
  for C,c in enumerate(r[:-E]):
   if(c!=M)>sum(sum(A[1+C:C+E-1])for A in g[1+B:B+D-1]):
    ...
    return g
# task 209: 276 (427 unzipped) vs 289 bytes for gold, https://arcprize.org/play?task=8a004b2b
def p(g):
 G=g[-5:]
...
# task 246: 111 vs 105 bytes for gold, https://arcprize.org/play?task=a2fd1cf0
...
# task 335: 118 vs 107 bytes for gold, https://arcprize.org/play?task=d4a91cb9
...
# task 370: 268 (357 unzipped) vs 260 bytes for gold, https://arcprize.org/play?task=e8dc4411
def p(g,k=3):
...
# task 390: 114 vs 99 bytes for gold, https://arcprize.org/play?task=f8a8fe49
p=lambda g,k=-1:k*g or p([*zip(*[g[a+sum(k*2for k in(2,3,-2,-3)if(g*2)[a+k].count(2)>4)]for a in range(15)])],k+1)
