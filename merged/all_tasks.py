# task 1: 61 vs 56 bytes for gold, https://arcprize.org/play?task=007bbfb7
...
# task 128: 62 vs 61 bytes for gold, https://arcprize.org/play?task=5521c0d9
 ...
 *C,M={i*90+j:g for i,g in enumerate(g)for j,g in enumerate(g)if g},
 ...
 return g
# task 134: 174 vs 168 bytes for gold, https://arcprize.org/play?task=5ad4f10b
...
# task 219: 277 (325 unzipped) vs 257 bytes for gold, https://arcprize.org/play?task=90f3ed37
def p(i,n=0,k=0):
...
# task 255: 236 (267 unzipped) bytes, gold, https://arcprize.org/play?task=a64e4611
def p(g):
 for S in[{0,3}]*8:g=[[g[y][~x]+10*any({*s[-2%(30-x):31-x]}-S for s in g[y+y%~y:y+2])for y in range(30)]for x in range(30)];g=[[r[x]%10|3*({*r[:10]}<=S)*(len(w:=[r[x]for r in g if{*r[:10]}<=S])>3!=S>={*w}or 3in r[x:])for x in range(30)]for r in g]
 return g
...
# task 336: 98 vs 93 bytes for gold, https://arcprize.org/play?task=d4f3cd78
