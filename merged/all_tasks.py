# task 1: 61 vs ??? bytes for gold, https://arcprize.org/play?task=007bbfb7
p=lambda a:[[d&e for d in b for e in c]for b in a for c in a]
# task 2: 98 vs ??? bytes for gold, https://arcprize.org/play?task=00d62c1b
p=lambda a,n=-62:[*map(lambda*b,d=0:[(c%2|d//3)*(d:=c+(n>c)*4)for c in b][::-1],*n*a or p(a,n+1))]
# task 3: 60 vs ??? bytes for gold, https://arcprize.org/play?task=017c7c7b
p=lambda g:[[v*2for v in l]for l in g+g[g[:2]!=g[4:]:][2:5]]
# task 4: 86 vs ??? bytes for gold, https://arcprize.org/play?task=025d127b
import re
p=lambda i:eval(re.sub(r"(([^0])(, .)+), 0(?=.*\2.*\].*\2)",r"0,\1",str(i)))
# task 5: 226 (244 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=045e512c
W=range(21)
t=-4,0,4
...
# task 6: 51 vs ??? bytes for gold, https://arcprize.org/play?task=0520fde7
p=lambda a:[[2*c*b.pop(0)for c in b[4:]]for b in a]
# task 7: 67 vs ??? bytes for gold, https://arcprize.org/play?task=05269061
p=lambda a,*b:[[max(sum(a,b:=[*b,0,0])[2::3])for _ in a]for _ in a]
# task 8: 94 vs ??? bytes for gold, https://arcprize.org/play?task=05f2a901
p=lambda g:[g:=sorted(zip(*g[::-1]),key=lambda x,P={0}:any((P.add(8in x),*P)+x))for _ in g][3]
# task 9: 127 vs ??? bytes for gold, https://arcprize.org/play?task=06df4c85
E=enumerate
p=lambda g:[[v or max(({*r[:j]}&{*r[j:]}|{*c[:i]}&{*c[i:]})-{r[2]}|{0})for j,(v,*c)in E(zip(r,*g))]for i,r in E(g)]
# task 10: 72 vs ??? bytes for gold, https://arcprize.org/play?task=08ed6ac7
p=lambda a:[[sum(c<e*s for*s,in zip(*a))for*c,e in zip(*a,r)]for r in a]
# task 11: 160 vs ??? bytes for gold, https://arcprize.org/play?task=09629e4f
E=enumerate;R=0,4,8
...
# task 12: 169 vs ??? bytes for gold, https://arcprize.org/play?task=0962bcdd
E=enumerate
...
# task 13: 169 vs ??? bytes for gold, https://arcprize.org/play?task=0a938d79
...
# task 14: 76 vs ??? bytes for gold, https://arcprize.org/play?task=0b148d64
p=lambda i:[[y[0]for y in zip(x,*i)if len({*y})>2]for x in i if len({*x})>2]
# task 15: 107 vs ??? bytes for gold, https://arcprize.org/play?task=0ca9ddb6
p=lambda i,k=3,r=range(9):-k*i or p([[i[~b][a]|(i[-b][a-1]==2)*4+(i[-b][a]==1)*7for b in r]for a in r],k-1)
# task 16: 46 vs ??? bytes for gold, https://arcprize.org/play?task=0d3d703e
p=lambda g:[[b" 	"[c]for c in g[0]]]*3
# task 17: 120 vs ??? bytes for gold, https://arcprize.org/play?task=0dfd9992
p=lambda a,n=1:[*map(f:=lambda*b:[max(b[i%n::n])for i in range(21)],*map(f,*a))]*any(b[n:]==b[:-n]for b in a)or p(a,n+1)
# task 18: 332 (374 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=0e206a2e
E=enumerate
def p(g):
 ...
 return g
# task 19: 126 vs ??? bytes for gold, https://arcprize.org/play?task=10fcaaa3
E=enumerate
p=lambda g:[[v or 8*any([*[*g*2,[0]*9][y+d%5-1]*2,0][x+d%3-1]for d in b'-/29')for x,v in E(l*2)]for y,l in E(g*2)]
# task 20: 173 vs ??? bytes for gold, https://arcprize.org/play?task=11852cab
r=range(10)
...
# task 21: 62 vs ??? bytes for gold, https://arcprize.org/play?task=1190e5a7
p=lambda i,*n:[x for x in zip(*n or p(i,*i))if i!=(i:=x)][::2]
# task 22: 99 vs ??? bytes for gold, https://arcprize.org/play?task=137eaa0f
p=lambda i:[[*map(max,*[s[t:t+3]for t in range(144)if(s:=sum(i*2,i))[t-x]==5])]for x in[-12,-1,10]]
# task 23: 216 vs ??? bytes for gold, https://arcprize.org/play?task=150deff5
import re
...
# task 24: 62 vs ??? bytes for gold, https://arcprize.org/play?task=178fcbfb
p=lambda i:[[3%-~sum(x)or(2in y)*2for y in zip(*i)]for x in i]
# task 25: 153 vs ??? bytes for gold, https://arcprize.org/play?task=1a07d186
E=enumerate
...
# task 26: 52 vs ??? bytes for gold, https://arcprize.org/play?task=1b2d62fb
p=lambda a:[[8>>i+b.pop(0)for i in b[4:]]for b in a]
# task 27: 104 vs ??? bytes for gold, https://arcprize.org/play?task=1b60fb0c
R=range(10)
p=lambda g:[[(g[i][j]^-g[~i+(C:=sum(map(max,*g,*zip(*g)))%2)][~j+C])%3for j in R]for i in R]
# task 28: 70 vs ??? bytes for gold, https://arcprize.org/play?task=1bfc4729
p=lambda g:[((j%11*[max(g[j%8])]+[0]*8)*2)[:10]for j in b'b"b""ooWoW']
# task 29: 118 vs ??? bytes for gold, https://arcprize.org/play?task=1c786137
p=lambda a:min([d in sum(i:=[b[1:-1]for*b,in zip(*[b for*b,in zip(*a)if d in b])if{d}<{*b}],a),i]for d in sum(a,a))[1]
# task 30: 127 vs ??? bytes for gold, https://arcprize.org/play?task=1caeab9d
import re;p=lambda i:[eval(re.sub(r"((\d)(, \2)*)",r"*map(int.__mul__,x[h.index(1):],[\1])",str(h:=[*map(max,*i)])))for x in i]
# task 31: 47 vs ??? bytes for gold, https://arcprize.org/play?task=1cf80156
p=lambda a,*n:[*filter(any,zip(*n or p(a,*a)))]
# task 32: 39 vs ??? bytes for gold, https://arcprize.org/play?task=1e0a9b12
p=lambda a:[*zip(*map(sorted,zip(*a)))]
# task 33: 77 vs ??? bytes for gold, https://arcprize.org/play?task=1e32b0e9
p=lambda g:[[v+(v<V)*R[5]for v,V in zip(r,R[:6]*3)]for r,R in zip(g,g[:6]*3)]
# task 34: 181 vs ??? bytes for gold, https://arcprize.org/play?task=1f0c79e5
...
# task 35: 96 vs ??? bytes for gold, https://arcprize.org/play?task=1f642eb9
p=lambda i,k=3:-k*i or[[x[0**n<=y^8]or y for y,n in zip(x,[0,*x])]for x in zip(*p(i,k-1)[::-1])]
# task 36: 106 vs ??? bytes for gold, https://arcprize.org/play?task=1f85a75f
p=lambda i:[l for n in{*sum(i,[])}if len(l:=[[y[0]for y in zip(x,*i)if n in y]for x in i if n in x])<6][0]
# task 37: 109 vs ??? bytes for gold, https://arcprize.org/play?task=1f876c06
p=lambda g,a=-1:[[max(sum({*(f:=sum(g,[]))[(a:=a+9//d)::d]}&{*f[a::-d]})for d in(9,11))for _ in g]for _ in g]
# task 38: 52 vs ??? bytes for gold, https://arcprize.org/play?task=1fad071e
p=lambda g:[(str(g).count('1, 1')//2*[1]+[0]*5)[:5]]
# task 39: 65 vs ??? bytes for gold, https://arcprize.org/play?task=2013d3e2
p=lambda a:[b[:3]for*b,in zip(*filter(any,zip(*a)))if any(b)][:3]
# task 40: 97 vs ??? bytes for gold, https://arcprize.org/play?task=2204b7a8
p=lambda i,k=1:-k*i or p([[x[y]and min(i[-(4<y)])or x[y]for y in range(10)]for x in zip(*i)],k-1)
# task 41: 49 vs ??? bytes for gold, https://arcprize.org/play?task=22168020
p=lambda a,b=0:[[e|(b:=b^e)for e in r]for r in a]
# task 42: 169 vs ??? bytes for gold, https://arcprize.org/play?task=22233c11
Q=range(10)
...
# task 43: 57 vs ??? bytes for gold, https://arcprize.org/play?task=2281f1f4
p=lambda a:[[c*b[-1]//9|b.pop(0)for c in a[0]]for*b,in a]
# task 44: 283 (360 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=228f6490
def p(i,r=range(10)):
 for n in{*sum(i,[])}:
 ...
 return i
# task 45: 45 vs ??? bytes for gold, https://arcprize.org/play?task=22eb0ac0
p=lambda g:[10*r[:r[0]==r[9]]or r for r in g]
# task 46: 176 vs ??? bytes for gold, https://arcprize.org/play?task=234bbc79
...
# task 47: 55 vs ??? bytes for gold, https://arcprize.org/play?task=23581191
p=lambda g:[[sum({*r+c})%13for*c,in zip(*g)]for r in g]
# task 48: 136 vs ??? bytes for gold, https://arcprize.org/play?task=239be575
p=lambda i,r=1,k=40:-k*[[8-8*("2"in str(i))]]or p([[(y>=1==s)or y-r+(r:=y!=2<r*3)for s,y in zip([0,*x],x)]for x in zip(*i[::-1])],0,k-1)
# task 49: 87 vs ??? bytes for gold, https://arcprize.org/play?task=23b5c85d
p=lambda a:[b.count(e)*[e]for b in a if(e:=min(set(d:=sum(a,[]))-{0},key=d.count))in b]
# task 50: 96 vs ??? bytes for gold, https://arcprize.org/play?task=253bf280
p=lambda g,*G:[[v or(8in{*r[:j]}&{*r[j:]})*3for j,v in enumerate(r)]for*r,in zip(*G or p(g,*g))]
# task 51: 126 vs ??? bytes for gold, https://arcprize.org/play?task=25d487eb
p=lambda i,k=3:-k*i or[(l:=0)or[y+0*(l:=l|(1>a<y!=b)*b)or l for a,b,y in zip([0,1,*x],[0,*x],x)]for x in zip(*p(i,k-1))][::-1]
# task 52: 41 vs ??? bytes for gold, https://arcprize.org/play?task=25d8a9c8
p=lambda a:[3*[1//len({*b})*5]for b in a]
# task 53: 21 vs ??? bytes for gold, https://arcprize.org/play?task=25ff71a9
p=lambda a:(a+a)[2:5]
# task 54: 381 (589 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=264363fd
def p(g):
 ...
 return g
# task 55: 104 vs ??? bytes for gold, https://arcprize.org/play?task=272f95fa
p=lambda i,z=0:[[z:=z+1-((c:=0)in x)]and[-c+(c:=c+y)or[0,2,0,4,6,3,0,1,0][z*3+c%7]for y in x]for x in i]
# task 56: 40 vs ??? bytes for gold, https://arcprize.org/play?task=27a28665
p=lambda i:[[0**i[0][0]+0**i[0][2]*3^2]]
# task 57: 49 vs ??? bytes for gold, https://arcprize.org/play?task=28bf18c6
p=lambda a,*n:[*filter(any,zip(*n or p(a,*a)*2))]
# task 58: 127 vs ??? bytes for gold, https://arcprize.org/play?task=28e73c20
p=lambda i,k=42:[[y+(len(x)-k>=0==y+s<=a==k//4*2)*3for y,s in zip(x,[0,*x])]for a,x in enumerate(zip(*-k*i or p(i,k-1)))][::-1]
# task 59: 191 vs ??? bytes for gold, https://arcprize.org/play?task=29623171
...
# task 60: 48 vs ??? bytes for gold, https://arcprize.org/play?task=29c11459
p=lambda g:[r[:1]*5+[5*(l>0)]+5*[l]for*r,l in g]
# task 61: 93 vs ??? bytes for gold, https://arcprize.org/play?task=29ec7d0e
p=lambda i,k=7:-k*i or[[*map(max,x,x[:len({*sum(i,[0])})-1]*9)]for x in zip(*p(i,k-1))][::-1]
# task 62: 175 vs ??? bytes for gold, https://arcprize.org/play?task=2bcee788
...
# task 63: 79 vs ??? bytes for gold, https://arcprize.org/play?task=2bee17df
p=lambda g:[[v or(any(c)*any(r[1:-1])<1)*3for _,*c,_,v in zip(*g,r)]for r in g]
# task 64: 184 vs ??? bytes for gold, https://arcprize.org/play?task=2c608aff
...
# task 65: 104 vs ??? bytes for gold, https://arcprize.org/play?task=2dc579da
p=lambda i:[p:=len(i)//2+1]and[[min(y,key=y.count)for y in zip(x,t,x[p:],t[p:])]for x,t in zip(i,i[p:])]
# task 66: 351 (479 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=2dd70a9a
def p(g):
...
# task 67: 33 vs ??? bytes for gold, https://arcprize.org/play?task=2dee498d
p=lambda a:[b[:len(a)]for b in a]
# task 68: 157 vs ??? bytes for gold, https://arcprize.org/play?task=31aa019c
...
# task 69: 209 vs ??? bytes for gold, https://arcprize.org/play?task=321b1fc6
def p(a,i=-1):
 ...
 return a
# task 70: 114 vs ??? bytes for gold, https://arcprize.org/play?task=32597951
p=lambda i:[[y%8*(8in{*x}&{*(u:=[*zip(*i)])[b],*{*u[b-1]}&{*u[-~b%17]}})*3or y for b,y in enumerate(x)]for x in i]
# task 71: 235 (247 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=3345333e
...
# task 72: 61 vs ??? bytes for gold, https://arcprize.org/play?task=3428a4f5
p=lambda a:[[3*(c!=b.pop(0))for c in a.pop(0)]for b in a[7:]]
# task 73: 46 vs ??? bytes for gold, https://arcprize.org/play?task=3618c87e
p=lambda a:a[:2]+a[::3]+[[5-b*4for b in a[2]]]
# task 74: 106 vs ??? bytes for gold, https://arcprize.org/play?task=3631a71a
p=lambda i,k=2,r=range(30):-k*i or p([[min(i[a][b],i[b][a],(i+i[1::-1])[~b][a])for b in r]for a in r],k-1)
# task 75: 88 vs ??? bytes for gold, https://arcprize.org/play?task=363442ee
p=lambda i,r=range(9):[i[a][:4]+[i[a//3*3+1][b//3*3+5]*i[a%3][b%3]for b in r]for a in r]
# task 76: 306 (355 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=36d67576
E=enumerate
def p(g):
 ...
 return g
# task 77: 152 vs ??? bytes for gold, https://arcprize.org/play?task=36fdfd69
p=lambda i,k=39:-k*i or p([[((c:=y.count)(2)+c(4)>=2!=y[0])*4or y[0]for y in zip(x,[0,*x],[*x[1:],0],t)]for*x,t in zip(*i,[[0]*99,*zip(*i)])][::-1],k-1)
# task 78: 61 vs ??? bytes for gold, https://arcprize.org/play?task=3906de3d
p=lambda i:[*zip(*[sorted(x,key=0 .__eq__)for x in zip(*i)])]
# task 79: 123 vs ??? bytes for gold, https://arcprize.org/play?task=39a8645d
p=lambda i:max(t:=[h for a in range(144)if all(map(any,(h:=[x[a//12:][:3]for x in i[a%12:][:3]])+[*zip(*h)]))],key=t.count)
# task 80: 313 (465 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=39e1d7f9
def p(g):
...
# task 81: 95 vs ??? bytes for gold, https://arcprize.org/play?task=3aa6fb7a
import re
p=lambda m,i=3:-i*m or[*zip(*eval(re.sub("0(?=, 8.{19}8)","1",str(p(m,i-1)[::-1]))))]
# task 82: 50 vs ??? bytes for gold, https://arcprize.org/play?task=3ac3eb23
p=lambda i:[f:=i[0],[*map(max,f[1:]+[0],[0]+f)]]*3
# task 83: 40 vs ??? bytes for gold, https://arcprize.org/play?task=3af2c5a8
p=lambda a:[b+b[::-1]for b in a+a[::-1]]
# task 84: 64 vs ??? bytes for gold, https://arcprize.org/play?task=3bd67248
def p(i,x=1):i[-1][x]=4;i[~x][x]=2;return i[:~x]and p(i,x+1)or i
# task 85: 56 vs ??? bytes for gold, https://arcprize.org/play?task=3bdb4ada
p=lambda i:[i:=[f:=y*(x!=i or f<y)for y in x]for x in i]
# task 86: 251 vs ??? bytes for gold, https://arcprize.org/play?task=3befdf3e
import re
...
# task 87: 36 vs ??? bytes for gold, https://arcprize.org/play?task=3c9b0459
p=lambda a:[b[::-1]for b in a[::-1]]
# task 88: 109 vs ??? bytes for gold, https://arcprize.org/play?task=3de23699
p=lambda a,n=-3:n*a[1:-1]or p([a[1:],[[d and(b:=max(a[n%2]))for d in c]for c in zip(*a[:0:-1])]][e:=b>0],n+e)
# task 89: 292 (376 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=3e980e27
def p(g):
 B=[];e=enumerate
 for A,r in e(g):
  for x,F in e(r):
 ...
 return g
# task 90: 197 vs ??? bytes for gold, https://arcprize.org/play?task=3eda0437
e=enumerate
...
# task 91: 63 vs ??? bytes for gold, https://arcprize.org/play?task=3f7978a0
p=lambda a,n=46:a*~n or p([*zip(*a[(5in a[n%2-2])-2::-1])],n-1)
# task 92: 99 vs ??? bytes for gold, https://arcprize.org/play?task=40853293
p=lambda i,*n:[[y or max({*x[b:],0}&{*x[:b],0})for b,y in enumerate(x)]for x in zip(*n or p(i,*i))]
# task 93: 105 vs ??? bytes for gold, https://arcprize.org/play?task=4093f84a
import re
p=lambda g,k=11:-k*g or p(eval(re.sub(r"[^50],([, 0]+)5",r"\1 5,5",str([*zip(*g[::-1])]))),k-1)
# task 94: 137 vs ??? bytes for gold, https://arcprize.org/play?task=41e4d17e
p=lambda i,k=1:-k*i or p([[(y>7>x.count(1)<3==sum([*zip(*i)][a-1:a+2],x).count(1)%5)*6or y for y in x]for a,x in enumerate(zip(*i))],k-1)
# task 95: 92 vs ??? bytes for gold, https://arcprize.org/play?task=4258a5f9
p=lambda i,k=7:-k*i or p([[y&5or z+y>>1for y,z in zip(x,[0,*x])]for x in zip(*i[::-1])],k-1)
# task 96: 325 (373 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=4290ef0e
def p(g,*S):
...
# task 97: 115 vs ??? bytes for gold, https://arcprize.org/play?task=42a50994
p=lambda i,e=enumerate:[[y*(sum(sum([z[b-(b>0):b+2]for z in i[a-(a>0):a+2]],[]))>y)for b,y in e(x)]for a,x in e(i)]
# task 98: 101 vs ??? bytes for gold, https://arcprize.org/play?task=4347f46a
E=enumerate
p=lambda g:[[v*(0in[*r[j-1:j+2],*[*zip(*g)][j][i-1:i+2]])for j,v in E(r)]for i,r in E(g)]
# task 99: 150 vs ??? bytes for gold, https://arcprize.org/play?task=444801d8
...
# task 100: 88 vs ??? bytes for gold, https://arcprize.org/play?task=445eab21
p=lambda a:[[max(range(1,10),key=[sum({*b}&{*c})for c in zip(*a)for b in a].count)]*2]*2
# task 101: 335 (447 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=447fd412
...
# task 102: 206 vs ??? bytes for gold, https://arcprize.org/play?task=44d8ac46
import re
...
# task 103: 29 vs ??? bytes for gold, https://arcprize.org/play?task=44f52bb0
p=lambda a:[[a==a[::-1]or 7]]
# task 104: 94 vs ??? bytes for gold, https://arcprize.org/play?task=4522001f
p=lambda g,R=range(9):[[3*(B//4==A//4<2)for B in R][::~-g[1][0]//2]for A in R][::~-g[0][1]//2]
# task 105: 154 vs ??? bytes for gold, https://arcprize.org/play?task=4612dd53
A=any
p=lambda g,k=-3:k*g or p([[v or A([r*(m:=[*map(A,g)])[j],sum(map(bool,r))//4*m[j:]][A(m[:j])])*2for j,v in enumerate(r)]for r in zip(*g)][::-1],k+1)
# task 106: 75 vs ??? bytes for gold, https://arcprize.org/play?task=46442a0e
p=lambda i:(s:=[a+x for a,*x in zip(i,*i[::-1])])+[x[::-1]for x in s[::-1]]
# task 107: 194 vs ??? bytes for gold, https://arcprize.org/play?task=469497ad
...
# task 108: 58 vs ??? bytes for gold, https://arcprize.org/play?task=46f33fce
p=eval('lambda a:[[a '+"for a in a[1::2]for _ in' '*4]"*2)
# task 109: 91 vs ??? bytes for gold, https://arcprize.org/play?task=47c1f68c
p=lambda a:[*map(f:=lambda*b,i=len(a)//2:[c and b[i]for c in b[:i]+b[i-1::-1]],*map(f,*a))]
# task 110: 188 vs ??? bytes for gold, https://arcprize.org/play?task=484b58aa
...
# task 111: 61 vs ??? bytes for gold, https://arcprize.org/play?task=48d8fb45
p=lambda g:[(f:=sum(g,[]))[f.index(5)+d:][:3]for d in b'	
']
# task 112: 112 vs ??? bytes for gold, https://arcprize.org/play?task=4938f0c2
p=lambda i,k=1:-k*i or p([[y or(x*2)[[*map(max,i)].index(3)*2-b+1]for b,y in enumerate(x)]for x in zip(*i)],k-1)
# task 113: 25 vs ??? bytes for gold, https://arcprize.org/play?task=496994bd
p=lambda a:a[:5]+a[4::-1]
# task 114: 65 vs ??? bytes for gold, https://arcprize.org/play?task=49d1d64f
p=lambda i:[[0,*i[0],0],*[x[:1]+x+x[-1:]for x in i],[0,*i[-1],0]]
# task 115: 54 vs ??? bytes for gold, https://arcprize.org/play?task=4be741c5
s={}.fromkeys
p=lambda a:[*s(zip(*s(zip(*map(s,a)))))]
# task 116: 20 vs ??? bytes for gold, https://arcprize.org/play?task=4c4377d9
p=lambda a:a[::-1]+a
# task 117: 172 vs ??? bytes for gold, https://arcprize.org/play?task=4c5c2cf0
...
# task 118: 333 (412 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=50846271
def p(i,n=2,R=range):
...
# task 126: 57 vs ??? bytes for gold, https://arcprize.org/play?task=54d82841
p=lambda a:a[:-1]+[[4*(sum(c)<2*max(c))for c in zip(*a)]]
# task 127: 82 vs ??? bytes for gold, https://arcprize.org/play?task=54d9e175
p=lambda i,E=enumerate:[[5+(y<5)*i[a&12|1][b&12|1]for b,y in E(x)]for a,x in E(i)]
# task 128: 64 vs ??? bytes for gold, https://arcprize.org/play?task=5521c0d9
p=lambda i:[*zip(*[x[(j:=-x.count(0)):]+x[:j]for x in zip(*i)])]
# task 129: 47 vs ??? bytes for gold, https://arcprize.org/play?task=5582e5ca
p=lambda a:[[max(b:=sum(a,a),key=b.count)]*3]*3
# task 130: 65 vs ??? bytes for gold, https://arcprize.org/play?task=5614dbcf
p=lambda a:[[sum({*b[c:c+3]}-{5})for c in(0,3,6)]for b in a[::3]]
# task 131: 126 vs ??? bytes for gold, https://arcprize.org/play?task=56dc2b01
p=lambda a,n=-3:n*a or p([[*b[:[*b,1].index(~b[l:=len(a)]%5%3)],*b[l:],8,*[0]*l][l-1::-1]for*b,in zip(*a,*filter(any,a))],n+1)
# task 132: 109 vs ??? bytes for gold, https://arcprize.org/play?task=56ff96f3
p=lambda i,k=3:-k*i or p([[max({*i[a]}&{*y}|{*y[:a+1]}&{*y[a:]})for a in range(len(i))]for y in zip(*i)],k-1)
# task 133: 330 (475 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=57aa92db
def p(g):
 C=[];e=enumerate
 for A,r in e(g):
  for B,F in e(r):
 ...
 return g
# task 134: 209 vs ??? bytes for gold, https://arcprize.org/play?task=5ad4f10b
...
# task 135: 32 vs ??? bytes for gold, https://arcprize.org/play?task=5bd6f4ac
p=lambda a:[b[6:]for b in a[:3]]
# task 136: 113 vs ??? bytes for gold, https://arcprize.org/play?task=5c0a986e
p=lambda i,k=19,r=range(10):-k*i or p([[i[~b][~a]or(a*b>0)*i[-b][-a]&2-k%2&i[1-b][1-a]for b in r]for a in r],k-1)
# task 137: 143 vs ??? bytes for gold, https://arcprize.org/play?task=5c2c9af4
...
# task 138: 152 vs ??? bytes for gold, https://arcprize.org/play?task=5daaa586
...
# task 139: 100 vs ??? bytes for gold, https://arcprize.org/play?task=60b61512
p=lambda i,k=11,r=range(9):-k*i or p([[i[~b][a]or i[-b][a]*i[~b][a-1]%3*7for b in r]for a in r],k-1)
# task 140: 36 vs ??? bytes for gold, https://arcprize.org/play?task=6150a2bd
p=lambda a:[b[::-1]for b in a[::-1]]
# task 141: 106 vs ??? bytes for gold, https://arcprize.org/play?task=623ea044
p=lambda i,e=enumerate:[[max((k:=z+[0]*99)[t+b-a]|k[a+b-t]for t,z in e(i))for b,y in e(x)]for a,x in e(i)]
# task 142: 40 vs ??? bytes for gold, https://arcprize.org/play?task=62c24649
p=lambda a:[b+b[::-1]for b in a+a[::-1]]
# task 143: 189 vs ??? bytes for gold, https://arcprize.org/play?task=63613498
...
# task 144: 59 vs ??? bytes for gold, https://arcprize.org/play?task=6430c8c4
p=lambda a:[[3>>c+b.pop(0)for c in a.pop(0)]for b in a[5:]]
# task 145: 199 vs ??? bytes for gold, https://arcprize.org/play?task=6455b5f5
...
# task 146: 58 vs ??? bytes for gold, https://arcprize.org/play?task=662c240a
p=lambda i:(r:=i[:3])*([*map(list,zip(*r))]!=r)or p(i[3:])
# task 147: 88 vs ??? bytes for gold, https://arcprize.org/play?task=67385a82
p=lambda i,k=3:-k*i or[[y+-y%~z%8for y,z in zip(x,[0,*x])]for x in zip(*p(i,k-1))][::-1]
# task 148: 169 vs ??? bytes for gold, https://arcprize.org/play?task=673ef223
...
# task 149: 82 vs ??? bytes for gold, https://arcprize.org/play?task=6773b310
p=lambda i,r=[0,4,8]:[[9<sum(sum(z[b:b+3])for z in i[a:a+3])for b in r]for a in r]
# task 150: 30 vs ??? bytes for gold, https://arcprize.org/play?task=67a3c6ac
p=lambda i:[x[::-1]for x in i]
# task 151: 108 vs ??? bytes for gold, https://arcprize.org/play?task=67a423a3
def p(g):
 x=y=0
 for _ in g:x+=1>g[0][x];y+=0in g[y]
 for N in b"":g[y+N//3-6][x+N%3-1]=4
 return g
# task 152: 40 vs ??? bytes for gold, https://arcprize.org/play?task=67e8384a
p=lambda a:[r+r[::-1]for r in a+a[::-1]]
# task 153: 184 vs ??? bytes for gold, https://arcprize.org/play?task=681b3aeb
...
# task 154: 133 vs ??? bytes for gold, https://arcprize.org/play?task=6855a6e4
p=lambda i,k=3:-k*i or p([[*[*zip(*i[::-1])][(1<abs(h:=(r:=[*map(max,*i)]).index(2)-l)<4==r.count(2))*2*h+l]]for l in range(15)],k-1)
# task 155: 18 vs ??? bytes for gold, https://arcprize.org/play?task=68b16354
p=lambda a:a[::-1]
# task 156: 199 vs ??? bytes for gold, https://arcprize.org/play?task=694f12f3
...
# task 157: 342 (391 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=6a1e5592
t=b'\n'
def p(g):
 def Q(o,p):
  c=C[o:];(3,)<=c!=Q(o+1,p)
...
# task 158: 322 (441 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=6aa20dc0
def p(g):
 ...
 return g
# task 159: 185 vs ??? bytes for gold, https://arcprize.org/play?task=6b9890af
...
# task 160: 115 vs ??? bytes for gold, https://arcprize.org/play?task=6c434453
import re;p=lambda i,*n:eval(re.sub("1, 1, 1(.{26}), 0, 1(.{26}), 1,",r"0,2,0\1*2,2,2\2*0,2,0*",str(n or p(i,*i))))
# task 161: 109 vs ??? bytes for gold, https://arcprize.org/play?task=6cdd2623
p=lambda i:[[sum(({x[0]}&{x[-1]}|{s[0]}&{s[-1]})&{min(k:=sum(i,[]),key=k.count)})for s in zip(*i)]for x in i]
# task 162: 107 vs ??? bytes for gold, https://arcprize.org/play?task=6cf79266
import re;p=lambda i,k=2:-k*i or p(eval(re.sub("0, 0, 0(.{55})?"*3,r"1,1,1\1 1,1,1\2 1,1,1\3",str(i))),k-1)
# task 163: 143 vs ??? bytes for gold, https://arcprize.org/play?task=6d0160f0
R=0,4,8
e=enumerate
p=lambda a:[[b*(b==5)or sum({a[k+i%4][l+j%4]for k in R for l in R if 4==a[k+i//4][l+j//4]})for j,b in e(r)]for i,r in e(a)]
# task 164: 32 vs ??? bytes for gold, https://arcprize.org/play?task=6d0aefbc
p=lambda a:[b+b[::-1]for b in a]
# task 165: 169 vs ??? bytes for gold, https://arcprize.org/play?task=6d58a25d
...
# task 166: 65 vs ??? bytes for gold, https://arcprize.org/play?task=6d75e8bb
p=lambda a:[[d or any(b)*2*any(c)for*c,d in zip(*a,b)]for b in a]
# task 167: 85 vs ??? bytes for gold, https://arcprize.org/play?task=6e02f1e3
p=lambda i:[b:=[[0,0,5],[0,5,0],[5,0,0]],b[::-1],[[5]*3]+[[0]*3]*2][4-len({*str(i)})]
# task 168: 117 vs ??? bytes for gold, https://arcprize.org/play?task=6e19193c
import re
p=lambda i,k=39:-k*i or[*zip(*eval(re.sub(r"0((.{35})+, ([^0]).{28}\3, \3)",r"\3\1",str(p(i,k-1))))[::-1])]
# task 169: 134 vs ??? bytes for gold, https://arcprize.org/play?task=6e82a1ae
p=lambda i,k=39,t=0:-k*i or[[[y and y|e,-len(y)%5][k>38]if k else{*[t:=t+1]*y}for y,e in zip(x,x[:1]+x)]for x in zip(*p(i,k-1)[::-1])]
# task 170: 243 (279 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=6ecd11f4
def p(g):
...
# task 171: 54 vs ??? bytes for gold, https://arcprize.org/play?task=6f8cd79b
p=lambda a:[*map(f:=lambda*b:[8,*b[2:],8],*map(f,*a))]
# task 172: 20 vs ??? bytes for gold, https://arcprize.org/play?task=6fa7a44f
p=lambda a:a+a[::-1]
# task 173: 248 (277 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=72322fa7
def p(g):
 ...
 return g
# task 174: 105 vs ??? bytes for gold, https://arcprize.org/play?task=72ca375d
p=lambda g,c=1:(k:=(f:=lambda g:[r for*r,in zip(*g)if c in r])(f(g)))*(k==[x[::-1]for x in k])or p(g,c+1)
# task 175: 92 vs ??? bytes for gold, https://arcprize.org/play?task=73251a56
e=enumerate
p=lambda g:[[[C|g[B][A]or g[0][1],g[0][0]][B==A]for(B,C)in e(B)]for(A,B)in e(g)]
# task 176: 74 vs ??? bytes for gold, https://arcprize.org/play?task=7447852a
p=lambda g:[[*map(max,(([4]*3+[0]*9)*9)[a%8::a%3],g[a%5])]for a in b"7)a"]
# task 177: 55 vs ??? bytes for gold, https://arcprize.org/play?task=7468f01a
p=lambda g:[[*filter(abs,r)][::-1]for r in g if any(r)]
# task 178: 49 vs ??? bytes for gold, https://arcprize.org/play?task=746b3537
p=lambda a:a*-1*-1or[p(b)for b in a if a!=(a:=b)]
# task 179: 21 vs ??? bytes for gold, https://arcprize.org/play?task=74dd1130
p=lambda a:[*zip(*a)]
# task 180: 82 vs ??? bytes for gold, https://arcprize.org/play?task=75b8110e
p=lambda g:[[max(a,key=bool)for a in zip(x[4:],e,e[4:],x)]for x,e in zip(g,g[4:])]
# task 181: 67 vs ??? bytes for gold, https://arcprize.org/play?task=760b3cac
def p(a):
	for b in a[:3]:c=6>>a[3][3];b[c:c+3]=b[5:2:-1]
	return a
# task 182: 187 vs ??? bytes for gold, https://arcprize.org/play?task=776ffc46
def p(g):
 R=F=sum(g,[])
 while C:=[*{*F}][-2]:
  ...
  if~-C:R,W=N,C
 return g
# task 183: 94 vs ??? bytes for gold, https://arcprize.org/play?task=77fdfe62
def p(g):A=len(g);B=range(2,A-2);return[[g[C][B]%7*g[0-C*2//A][0-B*2//A]for B in B]for C in B]
# task 184: 101 vs ??? bytes for gold, https://arcprize.org/play?task=780d0b14
p=lambda i,k=0,s=[0]*99:[s+0*(s:=[*x])for x in zip(*k or p(i,i))if-~-any(x)*(s:=[*map(max,s,x)])]+[s]
# task 185: 162 vs ??? bytes for gold, https://arcprize.org/play?task=7837ac64
...
# task 186: 66 vs ??? bytes for gold, https://arcprize.org/play?task=794b24be
p=lambda i:[[2,2%-~(k:=sum(sum(i,[]))),k//3*2],[0,k//4*2,0],[0]*3]
# task 187: 107 vs ??? bytes for gold, https://arcprize.org/play?task=7b6016b9
p=lambda i,k=79:-k*i or[[y*(y!=2)or(z==3)*3or y or 2for y,z in zip(x,[3,*x])]for x in zip(*p(i,k-1)[::-1])]
# task 188: 75 vs ??? bytes for gold, https://arcprize.org/play?task=7b7f7511
p=lambda a:[a[:len(a)//2],c:=[b[:len(b)//2]for b in a]][a==[b*2for b in c]]
# task 189: 140 vs ??? bytes for gold, https://arcprize.org/play?task=7c008303
p=lambda i,e=enumerate:i[0][2]&i[2][0]>7and[[y and i[a//3][b//3]for b,y in e(x[3:])]for a,x in e(i[3:])]or[*zip(*p([*zip(*i[::-1])]))][::-1]
# task 190: 112 vs ??? bytes for gold, https://arcprize.org/play?task=7ddcd7ec
import re;p=lambda i,k=19:-k*i or p(eval(re.sub("0(?=.{34}([^0]), 0.{31}\\1)","\\1",str([*zip(*i[::-1])]))),k-1)
# task 191: 267 (359 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=7df24a62
def p(g):
 ...
 return g
# task 192: 155 vs ??? bytes for gold, https://arcprize.org/play?task=7e0986d6
...
# task 193: 98 vs ??? bytes for gold, https://arcprize.org/play?task=7f4411dc
import re
p=lambda m,*a:eval(re.sub(r"(?<=[0 ,]..)\d(?=..[0 ])",'0',f" {[*zip(*a or p(m,*m))]} "))
# task 194: 74 vs ??? bytes for gold, https://arcprize.org/play?task=7fe24cdd
p=lambda g:(t:=[a+b for*b,a in zip(*g[::-1],g)])+[e[::-1]for e in t[::-1]]
# task 195: 105 vs ??? bytes for gold, https://arcprize.org/play?task=80af3007
p=lambda a,n=-1:n*[[d&e for d in b for e in c]for b in a for c in a]or p([*filter(any,zip(*a[::3]))],n+1)
# task 196: 135 vs ??? bytes for gold, https://arcprize.org/play?task=810b9b61
p=lambda i,k=79:-k*i or p([[(k<32>y==1<s)*3or~-(k<32>y==4)*y*~-(s<1>y%2)or k//79*4for s,y in zip([0,*x],x)]for x in zip(*i[::-1])],k-1)
# task 197: 54 vs ??? bytes for gold, https://arcprize.org/play?task=82819916
p=lambda a:[[b[a[1].index(c)]for c in a[1]]for b in a]
# task 198: 122 vs ??? bytes for gold, https://arcprize.org/play?task=83302e8f
p=lambda a,n=-23:n*a or p([*map(lambda*b,d=1:[[c,4-(sum(25>>e&d for e in b)>9)][9>>c&(d:=c!=4)]for c in b][::-1],*a)],n+1)
# task 199: 102 vs ??? bytes for gold, https://arcprize.org/play?task=834ec97d
p=lambda g,E=enumerate:[[g[A-1][B]+4*any(any(A[B%2::2])for A in g[A:])for B,_ in E(x)]for A,x in E(g)]
# task 200: 110 vs ??? bytes for gold, https://arcprize.org/play?task=8403a5d5
p=lambda g:[*zip(*[*[(P:=[0]*~-(h:=len(g)))*h]*(m:=max(g)).index(c:=max(m)),*[C:=[c]*h,[5]+P,C,P+[5]]*h][:h])]
# task 201: 255 (306 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=846bdb03
def p(g):
 ...
 return p
# task 202: 109 vs ??? bytes for gold, https://arcprize.org/play?task=855e0971
p=lambda a:(len({*a[0]}-{0})<2)*[[*map(min,*[c for c in a if max(b)in c])]for b in a]or[*zip(*p([*zip(*a)]))]
# task 203: 67 vs ??? bytes for gold, https://arcprize.org/play?task=85c4e7cd
p=lambda i:[[i[g:=len(i)//2][g+i[g].index(y)]for y in x]for x in i]
# task 204: 108 vs ??? bytes for gold, https://arcprize.org/play?task=868de0fa
import re;p=lambda i:eval(re.sub(r"(?<!1, )1, ((0, )+)1(?!, 1)",r"1,*[len([\1])%2*5+2]*len([\1]),1",str(i)))
# task 205: 225 (235 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=8731374e
...
# task 206: 176 vs ??? bytes for gold, https://arcprize.org/play?task=88a10436
def p(g):
 ...
 return g
# task 207: 81 vs ??? bytes for gold, https://arcprize.org/play?task=88a62173
p=eval('lambda a:[[min(b:=sum(a,()),key=b.count)'+'for*a,in map(zip,a,a[3:])]'*2)
# task 208: 258 (290 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=890034e9
def p(g):
 e=enumerate;A=g
...
# task 209: 299 (406 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=8a004b2b
def p(g):
 R=range;G=g[-5:]
  ...
  for y in R(20):
   for x in R(20):
    if all(g[Y][X]in[0,4-4*(y<=Y<y+H>0<x<=X<x+W)or G[(Y-y)//s][(X-x)//s]]for Y in R(len(g))for X in R(len(g[0]))):
     ...
     return g
# task 210: 20 vs ??? bytes for gold, https://arcprize.org/play?task=8be77c9e
p=lambda a:a+a[::-1]
# task 211: 48 vs ??? bytes for gold, https://arcprize.org/play?task=8d5021e8
p=lambda g:[A[::-1]+A for A in(g[::-1]+g)*9][:9]
# task 212: 117 vs ??? bytes for gold, https://arcprize.org/play?task=8d510a79
p=lambda i,k=39:-k*i or[[x[b]or 5in x[:b]and 2-x[b-1]%5&1-x[b+(b<9)]for b in range(10)]for x in zip(*p(i,k-1)[::-1])]
# task 213: 111 vs ??? bytes for gold, https://arcprize.org/play?task=8e1813be
p=lambda i:len({*i[-1]})<=(n:=len({*i[0]}))>2and~-n*[[k for k in i[0]if k]]or[*zip(*p([*zip(*i[::-1])]))][::-1]
# task 214: 63 vs ??? bytes for gold, https://arcprize.org/play?task=8e5a5113
p=lambda a:[b[:4]+c[::-1]+a.pop()[3::-1]for*c,b in[*zip(*a,a)]]
# task 215: 45 vs ??? bytes for gold, https://arcprize.org/play?task=8eb1be9a
p=lambda g:([max(g[::3]),*g[4:6]]*9)[:len(g)]
# task 216: 125 vs ??? bytes for gold, https://arcprize.org/play?task=8efcae92
r=range(661)
p=lambda a:max([-(c:=sum(b:=[b[x>>5:y>>5]for b in a[x%32:y%32]],a).count)(0),c(2),-x,b]for x in r for y in r)[3]
# task 217: 95 vs ??? bytes for gold, https://arcprize.org/play?task=8f2ea7aa
p=lambda a,*n:[*filter(any,zip(*[[d&e for d in b for e in c]for b in n for c in a]or p(a,*a)))]
# task 218: 58 vs ??? bytes for gold, https://arcprize.org/play?task=90c28cc7
p=lambda a,*n:[*{b:0for b in zip(*n or p(a,*a))if any(b)}]
# task 219: 294 (307 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=90f3ed37
Z=zip
def p(a,h=0):
...
# task 220: 113 vs ??? bytes for gold, https://arcprize.org/play?task=913fb3ed
p=lambda i,e=enumerate:[[y or max(max(s[b-(b>0):b+2])for s in i[a-(a>0):a+2])*5%9for b,y in e(x)]for a,x in e(i)]
# task 221: 102 vs ??? bytes for gold, https://arcprize.org/play?task=91413438
def p(i):j=3*str(i).count("0");return[[(a//3*j+b<27-j)*i[a%3][b%3]for b in range(j)]for a in range(j)]
# task 222: 197 vs ??? bytes for gold, https://arcprize.org/play?task=91714a58
...
# task 223: 52 vs ??? bytes for gold, https://arcprize.org/play?task=9172f3a0
p=lambda a:eval("[[a "+"for a in a for _ in%s]"%a*2)
# task 224: 171 vs ??? bytes for gold, https://arcprize.org/play?task=928ad970
...
# task 225: 145 vs ??? bytes for gold, https://arcprize.org/play?task=93b581b8
p=lambda i,k=3,r=range(6):-k*i or[[p([*zip(*i[::-1])],k-1)[b][~a]+max([*[s for t in i[a%3:a]for s in t[b%3:b]if s][:-3],0])for b in r]for a in r]
# task 226: 159 vs ??? bytes for gold, https://arcprize.org/play?task=941d9a10
...
# task 227: 57 vs ??? bytes for gold, https://arcprize.org/play?task=94f9d214
p=lambda a:[[2>>c+c+b.pop(0)for c in a.pop(4)]for b in a]
# task 228: 189 vs ??? bytes for gold, https://arcprize.org/play?task=952a094c
...
def p(A):
 ...
 return A
# task 229: 73 vs ??? bytes for gold, https://arcprize.org/play?task=9565186b
p=lambda a:[[[5,c][c==max(d:=sum(a,a),key=d.count)]for c in b]for b in a]
# task 230: 126 vs ??? bytes for gold, https://arcprize.org/play?task=95990924
p=lambda i,k=3,e=enumerate:-k*i or p([[y or-7**k%5*(i[-b][a-1]*i[1-b][a-2]>9)for b,y in e(x)]for a,x in e(zip(*i[::-1]))],k-1)
# task 231: 43 vs ??? bytes for gold, https://arcprize.org/play?task=963e52fc
p=lambda a:[(r[:6]*4)[:len(r)*2]for r in a]
# task 232: 63 vs ??? bytes for gold, https://arcprize.org/play?task=97999447
p=lambda i:[[(0>(e:=y-e))*5or e for y in x]for x in i if[e:=0]]
# task 233: 329 (460 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=97a05b5b
def p(g):
 ...
 for y,x,s in w[::-1]:
  for _ in'_ in':
   ...
   s=*zip(*s[::-1]),
 return u
# task 234: 120 vs ??? bytes for gold, https://arcprize.org/play?task=98cf29f8
p=lambda i,k=3,h={0}:-k*i or p([*zip(*([x for x in i if len(h:=h|{*x})-3+sum(x)-max(x)]+i[-1:]*99)[:len(i)])][::-1],k-1)
# task 235: 62 vs ??? bytes for gold, https://arcprize.org/play?task=995c5fa3
p=lambda g:[[g[1][x]*sum(g[2][x:x+3])%13^8]*3for x in[1,6,11]]
# task 236: 62 vs ??? bytes for gold, https://arcprize.org/play?task=99b1bc43
p=lambda i:[[-y%5^s*3for s,y in zip(*x)]for x in zip(i,i[5:])]
# task 237: 67 vs ??? bytes for gold, https://arcprize.org/play?task=99fa7670
p=lambda a,c=0:[(b:=0)or[c:=(b:=b or d)for d in r+[c]]for*r,_ in a]
# task 238: 234 (247 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=9aec4887
...
# task 239: 108 vs ??? bytes for gold, https://arcprize.org/play?task=9af7a82c
p=lambda a,i=0:[b:=sum(a,[]),c:=b.count]*any(r:=sorted([e*(c(e)>i)for e in{*b}],key=c)[::-1])and[r]+p(a,i+1)
# task 240: 107 vs ??? bytes for gold, https://arcprize.org/play?task=9d9215db
p=lambda i,k=4,r=range(19):-k*i or p([[i[~b][a]|i[b][a]|i[b%2*(a<b<18-a)*a+2][a]for b in r]for a in r],k-1)
# task 241: 21 vs ??? bytes for gold, https://arcprize.org/play?task=9dfd6313
p=lambda a:[*zip(*a)]
# task 242: 54 vs ??? bytes for gold, https://arcprize.org/play?task=9ecd008a
p=lambda g:[r[~r.index(0)::-1][:3]for r in g if 0in r]
# task 243: 79 vs ??? bytes for gold, https://arcprize.org/play?task=9edfc990
p=lambda a,n=-79:a*n or[*zip(*eval(str(p(a,n+1)[::-1]).replace('1, 0','1,1')))]
# task 244: 65 vs ??? bytes for gold, https://arcprize.org/play?task=9f236235
p=lambda a,n=1:(a[n]!=a[0])*[b[::~n]for b in a][::1+n]or p(a,n+1)
# task 245: 142 vs ??? bytes for gold, https://arcprize.org/play?task=a1570a43
p=lambda i,k=15:-k*i or p([[y[1]%2*3or y[(f:=sum(i,[]).index)(3)//(w:=len(i[0]))<f(2)//w]%3for y in zip([0,*x],x)]for x in zip(*i)][::-1],k-1)
# task 246: 131 vs ??? bytes for gold, https://arcprize.org/play?task=a2fd1cf0
p=lambda i,k=1:-k*i or p([[y or(sum((t:=[*map(max,i)])[:b+1])*sum(t[b:])>0<k+2in x)*8for b,y in enumerate(x)]for x in zip(*i)],k-1)
# task 247: 96 vs ??? bytes for gold, https://arcprize.org/play?task=a3325580
p=lambda a:(m:=max(map(c:=(b:=sum(zip(*a),())).count,{*b}-{0})))*[[*{d:0for d in b if c(d)==m}]]
# task 248: 79 vs ??? bytes for gold, https://arcprize.org/play?task=a3df8b1e
def p(g,A=0,C=-1):
 for x in g[::-1]:x[A]=1;C*=-(A%~-len(x)<1)|1;A+=C
 return g
# task 249: 26 vs ??? bytes for gold, https://arcprize.org/play?task=a416b8f3
p=lambda a:[r*2for r in a]
# task 250: 127 vs ??? bytes for gold, https://arcprize.org/play?task=a48eeaf7
import re
p=lambda i,k=7:-k*i or p(eval(re.sub(r"5(((.{34}0)+)(.{32})|([0, ]+)), 2",r"0\2\5+5\4,2",str([*zip(*i[::-1])]))),k-1)
# task 251: 94 vs ??? bytes for gold, https://arcprize.org/play?task=a5313dff
p=lambda a,n=-42:[*map(lambda*b,d=0:[max(n,c*(d+(d:=c)>1))for c in b][::-1],*n*a or p(a,n+1))]
# task 252: 61 vs ??? bytes for gold, https://arcprize.org/play?task=a5f85a15
p=lambda a:[(i:=1)*[[c,c and 4][i:=1-i]for c in b]for b in a]
# task 253: 148 vs ??? bytes for gold, https://arcprize.org/play?task=a61ba2ce
...
# task 254: 103 vs ??? bytes for gold, https://arcprize.org/play?task=a61f2674
def p(i):k=*map(sum,zip(*i)),;return[[y and s//max(k)+min({*k}-{0})//s*2for y,s in zip(x,k)]for x in i]
# task 255: 274 (313 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=a64e4611
def p(g):
...
# task 256: 96 vs ??? bytes for gold, https://arcprize.org/play?task=a65b410d
def p(g):s=sum(m:=max(g))//2;i=s-~g.index(m);return[[2-((i:=i+i%~i)<s)+(i>s)]*i+r[i:]for r in g]
# task 257: 81 vs ??? bytes for gold, https://arcprize.org/play?task=a68b268e
p=eval('lambda a:[[next(filter(int,sum(a,())),0)'+'for*a,in map(zip,a,a[5:])]'*2)
# task 258: 61 vs ??? bytes for gold, https://arcprize.org/play?task=a699fb00
import re
p=lambda a:eval(re.sub('1, 0(?=, 1)','1,2',str(a)))
# task 259: 85 vs ??? bytes for gold, https://arcprize.org/play?task=a740d043
p=lambda i,k=39:-k*i or p([*zip(*eval(str(i).replace(*"10"))[any(i[-1])-2::-1])],k-1)
# task 260: 150 vs ??? bytes for gold, https://arcprize.org/play?task=a78176bb
r=range(10)
p=lambda a:[[(c:=max({*max(a)}-{5}))*(c==a[i][j]or sum(m-n-i+j+k%5-2==0<a[m][n]for m in r for n in r for k in r)==2)for j in r]for i in r]
# task 261: 47 vs ??? bytes for gold, https://arcprize.org/play?task=a79310a0
p=lambda a:[[e%3for e in r]for r in[a.pop()]+a]
# task 262: 39 vs ??? bytes for gold, https://arcprize.org/play?task=a85d4709
p=lambda i:[[~b//~a^2]*3for a,b,_ in i]
# task 263: 122 vs ??? bytes for gold, https://arcprize.org/play?task=a87f7484
p=lambda g:g[(T:=[*zip(*[(x>0for y in g for x in y)]*9)]).index(min(T,key=T.count))*3:][:3%len(g)]or[*zip(*p([*zip(*g)]))]
# task 264: 216 vs ??? bytes for gold, https://arcprize.org/play?task=a8c38be5
...
# task 265: 149 vs ??? bytes for gold, https://arcprize.org/play?task=a8d7556c
def p(i):
 for n in range(289):
  for x in i[(a:=n//17)+3%~-sum(i[a][(b:=n%17):b+2]+i[a+1][b:b+2]):a+2]*(sum(i[1])!=49or a!=8):x[b:b+2]=2,2
 return i
# task 266: 103 vs ??? bytes for gold, https://arcprize.org/play?task=a9f96cdd
p=lambda g:[([0,0,0,r%9,0,r//9]*2)[4-max(g).index(2):][:5]for r in b'\09\0G\0'[2-g.index(max(g)):][:3]]
# task 267: 52 vs ??? bytes for gold, https://arcprize.org/play?task=aabf363d
p=lambda i:[[i[-y>>8][x==i[6]]for y in x]for x in i]
# task 268: 286 (327 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=aba27056
def p(g,k=3,v=range):
...
# task 269: 64 vs ??? bytes for gold, https://arcprize.org/play?task=ac0a08a4
p=lambda a:eval('[[a '+"for a in a for _ in[*{*'%s'}][5:]]"%a*2)
# task 270: 119 vs ??? bytes for gold, https://arcprize.org/play?task=ae3edfdc
import re;p=lambda i,k=7:-k*i or eval(re.sub(f"({k|3})([, \d]*)0(, {2-k//4})",r"0\2\1\3",str([*zip(*p(i,k-1)[::-1])])))
# task 271: 86 vs ??? bytes for gold, https://arcprize.org/play?task=ae4f1146
p=eval(f"lambda a:max([str(a).count('1'),a]{'for*a,in map(zip,a,a[1:],a[2:])'*2})[1]")
# task 272: 95 vs ??? bytes for gold, https://arcprize.org/play?task=aedd82e4
p=lambda a,n=7:-n*a or[*map(lambda*b,d=0:[2*(d*(d:=c)>0)or c-(c>n)for c in b][::-1],*p(a,n-1))]
# task 273: 116 vs ??? bytes for gold, https://arcprize.org/play?task=af902bf9
p=lambda i,k=3:-k*i or p([[(y%2<=k)*(y or~-max({*x[:b],1}&{*x[b:],1}))for b,y in enumerate(x)]for x in zip(*i)],k-1)
# task 274: 75 vs ??? bytes for gold, https://arcprize.org/play?task=b0c4d837
p=lambda i:[(h:=[8]*sum({*x}=={0,5}for x in i)+[0]*5)[1:4],h[6:3:-1],[0]*3]
# task 275: 137 vs ??? bytes for gold, https://arcprize.org/play?task=b190f7f5
p=lambda i:"8"in str(i[(u:=len(i[0])):])and[[t*y/8for t in s for y in x]for s in i[:u]for x in i[u:]]or[*zip(*p([*zip(*i[::-1])]))][::-1]
# task 276: 38 vs ??? bytes for gold, https://arcprize.org/play?task=b1948b0a
p=lambda a:eval(str(a).replace(*'62'))
# task 277: 198 vs ??? bytes for gold, https://arcprize.org/play?task=b230c067
def p(g,*M):
...
']];M+=s,;g[i//30][i%10]=3-M[:len(A)].count(s)
 return g
# task 278: 143 vs ??? bytes for gold, https://arcprize.org/play?task=b27ca6d3
...
# task 285: 317 (400 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=b775ac94
def p(g,k=7):
 I=[]
 ...
 return-k*g or p([*map(list,zip(*g[::-1]))],k-1)
# task 286: 121 vs ??? bytes for gold, https://arcprize.org/play?task=b782dc8a
p=lambda i,k=139:-k*i or[[z[1]or([0,*{*z,8}^{*sum(i,[])}]*2)[3]for z in zip([0,*x],x,[*x[1:],0])]for x in zip(*p(i,k-1))]
# task 287: 63 vs ??? bytes for gold, https://arcprize.org/play?task=b8825c91
p=lambda a:[[max({c,b.pop()}-{4})for c in a.pop()]for*b,in[*a]]
# task 288: 112 vs ??? bytes for gold, https://arcprize.org/play?task=b8cdaf2b
p=lambda i,e=enumerate:[[y or i[-1][t:=len(i)//2]*(a+abs(b-t)==t*2-0**i[-2][-t])for b,y in e(x)]for a,x in e(i)]
# task 289: 64 vs ??? bytes for gold, https://arcprize.org/play?task=b91ae062
p=lambda a:eval('[[a '+"for a in a for _ in[*{*'%s'}][5:]]"%a*2)
# task 290: 69 vs ??? bytes for gold, https://arcprize.org/play?task=b94a9452
p=lambda i:[[sum({*sum(i,x)},-y)for y in x if y]for x in i if any(x)]
# task 291: 62 vs ??? bytes for gold, https://arcprize.org/play?task=b9b7f026
p=lambda i,n=1:len({x.count(n)for x in i})//3*[[n]]or p(i,n+1)
# task 292: 58 vs ??? bytes for gold, https://arcprize.org/play?task=ba26e723
p=lambda i:[[8%~y&6-b%3for b,y in enumerate(x)]for x in i]
# task 293: 60 vs ??? bytes for gold, https://arcprize.org/play?task=ba97ae07
p=lambda a:[[d^c^b[0]or d for*_,c,d in zip(*a,b)]for b in a]
# task 294: 76 vs ??? bytes for gold, https://arcprize.org/play?task=bb43febb
import re
p=lambda i:eval(re.sub("(?<=5.{28}5, )5(?=, 5.{28}5)","2",str(i)))
# task 295: 54 vs ??? bytes for gold, https://arcprize.org/play?task=bbc9ae5d
p=lambda a:[b:=a[0]]+[b:=b[:1]+b[:-1]for _ in b[2::2]]
# task 296: 63 vs ??? bytes for gold, https://arcprize.org/play?task=bc1d5164
p=lambda i:[[*map(max,x,s,x[4:],s[4:])]for x,s in zip(i,i[2:])]
# task 297: 44 vs ??? bytes for gold, https://arcprize.org/play?task=bd4472b8
p=lambda i:i[:2]+[*zip(*[i[0]*2]*len(i[0]))]
# task 298: 55 vs ??? bytes for gold, https://arcprize.org/play?task=bda2d7a6
p=lambda i:[[i[3][~-x.index(y)%3]for y in x]for x in i]
# task 299: 54 vs ??? bytes for gold, https://arcprize.org/play?task=bdad9b1f
p=lambda i:[[3%~t+2&8-(2in x)for t in i[0]]for x in i]
# task 300: 87 vs ??? bytes for gold, https://arcprize.org/play?task=be94b721
p=lambda a,*n:[b for*b,in zip(*n or p(a,*a))if max(range(1,10),key=sum(a,a).count)in b]
# task 301: 31 vs ??? bytes for gold, https://arcprize.org/play?task=beb8660c
p=lambda i,s=sorted:s(map(s,i))
# task 302: 93 vs ??? bytes for gold, https://arcprize.org/play?task=c0f76784
import re
p=lambda i:eval(re.sub("5,([ 0,]*)5(?=, 0|])",r"5,*[5+(r:=len([\1]))]*r,5",str(i)))
# task 303: 64 vs ??? bytes for gold, https://arcprize.org/play?task=c1d99e64
p=lambda a:[[d^2^2*any(b)*any(c)for*c,d in zip(*a,b)]for b in a]
# task 304: 92 vs ??? bytes for gold, https://arcprize.org/play?task=c3e719e8
p=lambda i:[[t*(y==max(z:=sum(i,i),key=z.count))for y in x for t in s]for x in i for s in i]
# task 305: 62 vs ??? bytes for gold, https://arcprize.org/play?task=c3f564a4
p=lambda g:[(sorted({*g[0]}-{0})*9)[y:y+16]for y in range(16)]
# task 306: 81 vs ??? bytes for gold, https://arcprize.org/play?task=c444b776
p=lambda i,k=7:-k*i or[[*x[:10],*map(max,x,x[10:])]for x in zip(*p(i,k-1))][::-1]
# task 307: 52 vs ??? bytes for gold, https://arcprize.org/play?task=c59eb873
p=eval('lambda a:[[a '+"for a in a for _ in'  ']"*2)
# task 308: 269 (316 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=c8cbb738
def p(g):
...
# task 309: 38 vs ??? bytes for gold, https://arcprize.org/play?task=c8f0f002
p=lambda a:eval(str(a).replace(*'75'))
# task 310: 96 vs ??? bytes for gold, https://arcprize.org/play?task=c909285e
import re
p=lambda m:[*map(eval,re.findall((c:=min(s:=str(m)+'[]'*8,key=s.count))+"[^[]*"+c,s))]
# task 311: 32 vs ??? bytes for gold, https://arcprize.org/play?task=c9e6f938
p=lambda a:[r+r[::-1]for r in a]
# task 312: 45 vs ??? bytes for gold, https://arcprize.org/play?task=c9f8e694
p=lambda a:[[e and r[0]for e in r]for r in a]
# task 313: 65 vs ??? bytes for gold, https://arcprize.org/play?task=caa06a1f
p=lambda g,x=1:[(g[x:=x^1][1:len({*g[0]})]*9)[:len(g)]for _ in g]
# task 314: 102 vs ??? bytes for gold, https://arcprize.org/play?task=cbded52d
p=lambda i,r=range(8):[[((x:=i[a])[b-5]>1)*x[b-3]|(i[a-5][b]>1)*i[a-3][b]or x[b]for b in r]for a in r]
# task 315: 63 vs ??? bytes for gold, https://arcprize.org/play?task=cce03e0d
p=lambda i:[[t&-y%5for y in x for t in s]for x in i for s in i]
# task 316: 72 vs ??? bytes for gold, https://arcprize.org/play?task=cdecee7f
p=lambda i:[(q:=sorted(map(max,*i),key=0 .__eq__))[:3],q[5:2:-1],q[6:9]]
# task 317: 59 vs ??? bytes for gold, https://arcprize.org/play?task=ce22a75a
p=lambda i,r=b"":[[i[a][b]>0for b in r]for a in r]
# task 318: 60 vs ??? bytes for gold, https://arcprize.org/play?task=ce4f8723
p=lambda i:[[any(y)*3for y in zip(*x)]for x in zip(i,i[5:])]
# task 319: 306 (392 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=ce602527
E=enumerate
def p(g):
...
# task 320: 72 vs ??? bytes for gold, https://arcprize.org/play?task=ce9e57f2
p=lambda a:[[c<<b.pop(0)for c in a.pop(0)]for*b,in(a[:1]*len(a)+a)[::2]]
# task 321: 57 vs ??? bytes for gold, https://arcprize.org/play?task=cf98881b
p=lambda m:[[r.pop(0)or r[4]or r[9]for _ in m]for r in m]
# task 322: 49 vs ??? bytes for gold, https://arcprize.org/play?task=d037b0a7
p=lambda a,b=[0]*9:[b:=[*map(max,b,r)]for r in a]
# task 323: 153 vs ??? bytes for gold, https://arcprize.org/play?task=d06dbe63
...
# task 324: 291 (343 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=d07ae81c
def p(g,e=enumerate):
 ...
 return g
# task 325: 184 vs ??? bytes for gold, https://arcprize.org/play?task=d0f5fe59
...
# task 326: 30 vs ??? bytes for gold, https://arcprize.org/play?task=d10ecb37
p=lambda a:[a[0][:2],a[1][:2]]
# task 327: 67 vs ??? bytes for gold, https://arcprize.org/play?task=d13f3404
p=lambda i,l=[0]*3:[l:=[*map(max,x+[0]*3,[0]+l*2)]for x in i+[l]*3]
# task 328: 188 vs ??? bytes for gold, https://arcprize.org/play?task=d22278a0
...
# task 350: 96 vs ??? bytes for gold, https://arcprize.org/play?task=dbc1a6ce
p=lambda i,*n:[[y or(1in x[b:]!=1in x[:b])*8for b,y in enumerate(x)]for x in zip(*n or p(i,*i))]
# task 351: 68 vs ??? bytes for gold, https://arcprize.org/play?task=dc0a314f
p=lambda i:[s[~x.index(3)::-1][:5]for x,s in zip(i,i[::-1])if 3in x]
# task 352: 90 vs ??? bytes for gold, https://arcprize.org/play?task=dc1df850
p=lambda i,k=3:-k*i or[[y or 6>>t&9for y,t in zip(x,[0,*x])]for x in zip(*p(i,k-1)[::-1])]
# task 353: 104 vs ??? bytes for gold, https://arcprize.org/play?task=dc433765
p=lambda a,n=-3,i=0:n*a or 3in a[i]and p([*zip(a.pop(('4'in'%s'%a[:i])*i-1),*a[::-1])],n+1)or p(a,n,i+1)
# task 354: 101 vs ??? bytes for gold, https://arcprize.org/play?task=ddf7fa4f
p=lambda i,k=9:-k*i or p([[(y==5)*(c[0]or s)or y for*c,y,s in zip(*i,x,[0,*x])][::-1]for x in i],k-1)
# task 355: 101 vs ??? bytes for gold, https://arcprize.org/play?task=de1cd16c
p=lambda a:[sorted(range(10),key=lambda c:sum(e!=c in{*b}&{*d}for b in a for*d,e in zip(*a,b)))[8:9]]
# task 356: 105 vs ??? bytes for gold, https://arcprize.org/play?task=ded97339
p=lambda i,r=range(10):[[max({*i[a][b:]}&{*i[a][:b+1]}|{*c[a:]}&{*c[:a]})for*c,b in zip(*i,r)]for a in r]
# task 357: 92 vs ??? bytes for gold, https://arcprize.org/play?task=e179c5f4
p=lambda m,x=1,d=-1:m and p(m[1:],x:=x+d,x%(l:=len(m[0])-1)and d or-d)+[[8]*x+[1]+[8]*(l-x)]
# task 358: 97 vs ??? bytes for gold, https://arcprize.org/play?task=e21d9049
p=lambda i,k=39:-k*i or[[*map(max,x,x[6-13//len({*x,0}):]+(0,0)*9)]for x in zip(*p(i,k-1)[::-1])]
# task 359: 64 vs ??? bytes for gold, https://arcprize.org/play?task=e26a3af2
p=lambda g:[[max(w:=c+r,key=w.count)for*c,in zip(*g)]for r in g]
# task 360: 45 vs ??? bytes for gold, https://arcprize.org/play?task=e3497940
p=lambda a:[[*map(max,b,b[:4:-1])]for b in a]
# task 361: 220 (235 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=e40b9e2f
def p(i,n=4,r=range(10)):
 ...
 return p(i,n-1)
# task 362: 92 vs ??? bytes for gold, https://arcprize.org/play?task=e48d4e1a
p=lambda i,r=range(10):[[(i[a-(s:=str(i).count("5"))][b-10+s]^5or 5)^5for b in r]for a in r]
# task 363: 243 (300 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=e5062a87
def p(g):
 ...
 return g
# task 364: 163 vs ??? bytes for gold, https://arcprize.org/play?task=e509e548
...
# task 365: 134 vs ??? bytes for gold, https://arcprize.org/play?task=e50d258f
p=lambda i:max([[x[a%9:][:9-a//9%9]for x in i[a//81%9:][:9-a//729]]for a in range(9**4)],key=lambda p:"0"in(r:=str(p))or r.count("2"))
# task 366: 426 (612 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=e6721834
def p(g):
...
# task 367: 225 vs ??? bytes for gold, https://arcprize.org/play?task=e73095fd
...
# task 368: 160 vs ??? bytes for gold, https://arcprize.org/play?task=e76a88a6
import re
def p(i):
 def t(m,q=[-1]*99):q[y:=m.end()%32]+=1;return re.findall("([^50](, [1-46-9])+)",str(i)*9)[q[y]][0]
 return eval(re.sub("5(, 5)+",t,str(i)))
# task 369: 118 vs ??? bytes for gold, https://arcprize.org/play?task=e8593010
p=lambda m,i=91:-i*m or[*zip(*eval(str(p(m,i-1)[::-1]).replace("3220,,,,    331"[i%7%4::4],"2113,,,,211"[i%7%4::4])))]
# task 370: 277 (342 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=e8dc4411
def p(g,k=3):
...
# task 371: 115 vs ??? bytes for gold, https://arcprize.org/play?task=e9614598
def p(a):
	b=sum(a,[]).index;c=b(1);d=b(1,c+1)+c>>1
	for i in[l:=len(a[0]),~l,1,1,~l]:d+=i;a[d//l][d%l]=3
	return a
# task 372: 48 vs ??? bytes for gold, https://arcprize.org/play?task=e98196ab
p=lambda a:[b for*b,in map(map,[max]*5,a,a[6:])]
# task 373: 39 vs ??? bytes for gold, https://arcprize.org/play?task=e9afcf9a
p=lambda a:[b:=[*map(max,a)]*3,b[::-1]]
# task 374: 155 vs ??? bytes for gold, https://arcprize.org/play?task=ea32f347
...
# task 375: 53 vs ??? bytes for gold, https://arcprize.org/play?task=ea786f4a
def p(g,i=0):
 for r in g:r[i]=r[~i]=0;i+=1
 return g
# task 376: 30 vs ??? bytes for gold, https://arcprize.org/play?task=eb281b96
p=lambda a:(a+a[1:-1])*2+a[:1]
# task 377: 57 vs ??? bytes for gold, https://arcprize.org/play?task=eb5a1d5d
p=lambda g,*a:[y for*y,in zip(*a or p(g,*g))if g!=(g:=y)]
# task 378: 244 (250 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=ec883f72
def p(g):
 ...
 return g
# task 379: 142 vs ??? bytes for gold, https://arcprize.org/play?task=ecdecbb3
import re
p=lambda i,k=7,r=re.sub:-k*i or p(eval(r(", 4, ","|8,8,8|",r("0, 8, ((0, )+)2","4,2,4,*[2]*len([\\1])",str([*zip(*i[::-1])])))),k-1)
# task 380: 27 vs ??? bytes for gold, https://arcprize.org/play?task=ed36ccf7
p=lambda a:[*zip(*a)][::-1]
# task 381: 100 vs ??? bytes for gold, https://arcprize.org/play?task=ef135b50
import re
p=lambda i:i[:1]+eval(re.sub("2, ((0, )+)(?=2)","2,*[9]*len([\\1]),",str(i[1:-1])))+i[-1:]
# task 382: 145 vs ??? bytes for gold, https://arcprize.org/play?task=f15e1fac
f=lambda b,*a:[b:=(*b[d>0:-1],*{0,d})for*_,d in[b,*a]]
p=lambda a,n=-3:n*a or p([b:=(c:=[*zip(*a)])[::-1],max(f(*c)[::-1],f(*b))][2in a[-1]],n+1)
# task 383: 215 vs ??? bytes for gold, https://arcprize.org/play?task=f1cefba8
...
# task 384: 65 vs ??? bytes for gold, https://arcprize.org/play?task=f25fbde4
p=lambda a,*n:[b for*b,in zip(*n or p(a,*a))for _ in'  'if 4in b]
# task 385: 25 vs ??? bytes for gold, https://arcprize.org/play?task=f25ffba3
p=lambda a:a[:4:-1]+a[5:]
# task 386: 52 vs ??? bytes for gold, https://arcprize.org/play?task=f2829549
p=lambda a:[[3>>e+r.pop(0)for e in r[4:]]for r in a]
# task 387: 234 vs ??? bytes for gold, https://arcprize.org/play?task=f35d900a
...
# task 388: 62 vs ??? bytes for gold, https://arcprize.org/play?task=f5b8619d
p=lambda a:2*[2*[d or any(c)*8for*c,d in zip(*a,b)]for b in a]
# task 389: 57 vs ??? bytes for gold, https://arcprize.org/play?task=f76d97a5
p=lambda a:[[sum({*sum(a,r)}-{e,5})for e in r]for r in a]
# task 390: 147 vs ??? bytes for gold, https://arcprize.org/play?task=f8a8fe49
p=lambda i,k=3,r=range(15):-k*i or p([[*[*zip(*i[::-1])][(1<abs((s:=[n-l for n in r if 2in[*zip(*i)][n]])[0])<4==len(s))*2*s[0]+l]]for l in r],k-1)
# task 391: 63 vs ??? bytes for gold, https://arcprize.org/play?task=f8b3ba0a
p=lambda m:[*zip(sorted({*(a:=sum(m,[]))},key=a.count))][2::-1]
# task 392: 221 vs ??? bytes for gold, https://arcprize.org/play?task=f8c80d96
def p(g):
 ...
 return g
# task 393: 63 vs ??? bytes for gold, https://arcprize.org/play?task=f8ff0b80
p=lambda m:[*zip(sorted(set(a:=sum(m,[])),key=a.count))][2::-1]
# task 394: 128 vs ??? bytes for gold, https://arcprize.org/play?task=f9012d9b
p=lambda i,n=1:all(len({*x[::n],0})<3for x in i)and[[max(x[b%n::n])for b,y in enumerate(x)if y<1]for x in i if 0in x]or p(i,n+1)
# task 395: 57 vs ??? bytes for gold, https://arcprize.org/play?task=fafffa47
p=lambda a:[[2>>c+c+b.pop(0)for c in a.pop(3)]for b in a]
# task 396: 261 (268 unzipped) vs ??? bytes for gold, https://arcprize.org/play?task=fcb5c309
def p(m):
 ...
 return z[1]
# task 397: 130 vs ??? bytes for gold, https://arcprize.org/play?task=fcc82909
p=lambda m,k=0:exec('r=k//9;c=k%9;exec("m[r+2][c:c+2]=3,3;r+=1;"*len(a:={*m[r][c:c+2]+m[r+1][c:c+2]})*(not{0,3}&a));k+=1;'*81)or m
# task 398: 80 vs ??? bytes for gold, https://arcprize.org/play?task=feca6190
def p(a):l=25-5*a[0].count(0);return[((~-l*[0]+a[0])*2)[i:i+l]for i in range(l)]
# task 399: 64 vs ??? bytes for gold, https://arcprize.org/play?task=ff28f65a
p=lambda m:[[1,0,(c:=sum(sum(m,[]))/8)>1],[0,c>2,0],[c>3,0,c>4]]
# task 400: 70 vs ??? bytes for gold, https://arcprize.org/play?task=ff805c23
