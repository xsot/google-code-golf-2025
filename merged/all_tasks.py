...
# 🥈 task 40: 70 vs 69 bytes for gold, https://arcprize.org/play?task=2204b7a8
...
# ❌ task 58: 111 vs 103 bytes for gold, https://arcprize.org/play?task=28e73c20
...
# 💎 task 66: 247 (341 unzipped) bytes, gold, https://arcprize.org/play?task=2dd70a9a
def	p(g):
...
# 💎 task 76: 272 (384 unzipped) bytes, gold, https://arcprize.org/play?task=36d67576
def	p(g):
...
# 💎 task 101: 263 (444 unzipped) bytes, gold, https://arcprize.org/play?task=447fd412
def p(g):
 ...
 return g
# 💎 task 102: 142 bytes, gold, https://arcprize.org/play?task=44d8ac46
...
# 💎 task 145: 173 bytes, gold, https://arcprize.org/play?task=6455b5f5
...
# 💎 task 158: 262 (488 unzipped) bytes, gold, https://arcprize.org/play?task=6aa20dc0
def p(g):
...
# 🥇 task 170: 196 (236 unzipped) bytes, gold, https://arcprize.org/play?task=6ecd11f4
...
p=lambda a:a+a[::-1]
# 💎 task 173: 207 (298 unzipped) bytes, gold, https://arcprize.org/play?task=72322fa7
p=lambda n:[0for l,s in enumerate(n)for d,s in enumerate(s)for y,s in enumerate(n)for t,s in enumerate(s)if((r:=[n[y-f//3][t-f%3]for f,s in enumerate(n[:9])])==r[::-1])*any(r[4]*r[:4])*(r[4]==n[l-1][d-1]or sum(s==n[l-f//3][d-f%3]for f,s in enumerate(r))>7)for f,n[l-f//3][d-f%3]in enumerate(r)]*0+n
...
# 💎 task 191: 231 (384 unzipped) bytes, gold, https://arcprize.org/play?task=7df24a62
def p(g):
...
# 🥇 task 196: 111 bytes, gold, https://arcprize.org/play?task=810b9b61
...
# 🥇 task 219: 244 (261 unzipped) bytes, gold, https://arcprize.org/play?task=90f3ed37
def p(g):
 q=H=i=0
 while g[i:]:
...
# 💎 task 233: 285 (456 unzipped) bytes, gold, https://arcprize.org/play?task=97a05b5b
def	p(g):
...
# 🥈 task 273: 116 vs 109 bytes for gold, https://arcprize.org/play?task=af902bf9
R=range(10)
...
# 💎 task 277: 158 bytes, gold, https://arcprize.org/play?task=b230c067
z=[0]
...
# 🥉 task 279: 110 vs 107 bytes for gold, https://arcprize.org/play?task=b2862040
p=lambda g,f=126:~f*g or p([*map(lambda*r,a=9:[9&b%[9|12-a,a,a:=9+b][f>>6]or(f<0)*9for b in r],*g[::-1])],f-1)
# 💎 task 280: 177 bytes, gold, https://arcprize.org/play?task=b527c5c6
def p(a,n=3,i=0):
 for b in a:
...
# 🥈 task 284: 205 vs 201 bytes for gold, https://arcprize.org/play?task=b7249182
...
# 💎 task 285: 275 (431 unzipped) bytes, gold, https://arcprize.org/play?task=b775ac94
def	p(g):
	for*I,in[[]]*8:
...
p=lambda a:[*zip(*a)][::-1]
# 🥈 task 381: 81 vs 79 bytes for gold, https://arcprize.org/play?task=ef135b50
...
# 🥈 task 387: 207 vs 202 bytes for gold, https://arcprize.org/play?task=f35d900a
