...
# 🥈 task 2: 88 vs 86 bytes for gold, https://arcprize.org/play?task=00d62c1b
...
# 🥉 task 5: 199 (259 unzipped) vs 166 bytes for gold, https://arcprize.org/play?task=045e512c
...
# ❌ task 6: 51 vs 49 bytes for gold, https://arcprize.org/play?task=0520fde7
...
# ❌ task 9: 109 vs 95 bytes for gold, https://arcprize.org/play?task=06df4c85
p=lambda g,E=enumerate:[[max({*r[:j+1]}&{*r[j::3]}|{*c[:i]}&{*c[i::3]})for j,c in E(zip(*g))]for i,r in E(g)]
# 🥈 task 10: 67 vs 66 bytes for gold, https://arcprize.org/play?task=08ed6ac7
...
# 🥉 task 12: 129 vs 122 bytes for gold, https://arcprize.org/play?task=0962bcdd
import re;p=lambda i:exec(r'i[::-1]=zip(*eval(re.sub("(([^0]).{37}([^0]), )0(, 0.{31})0, 0,",r"\1\2\4\3,0,\2+",str(i))));'*8)or i
# 🥉 task 13: 136 vs 124 bytes for gold, https://arcprize.org/play?task=0a938d79
def p(i):*v,=map(max,*i);*_,f,r=sorted(map(v.index,v));N=len(v);return i[N:]and[*zip(*p([*zip(*i)]))]or[(v[:f]+v[f:2*r-f]*N)[:N]]*len(i)
# 🥉 task 14: 68 vs 57 bytes for gold, https://arcprize.org/play?task=0b148d64
p=lambda i,*I:[w for*w,r in zip(*I or p(zip(*i),*i),i)if[*{*r}][2:]]
# 🥈 task 15: 92 vs 87 bytes for gold, https://arcprize.org/play?task=0ca9ddb6
...
# 🥉 task 17: 102 vs 90 bytes for gold, https://arcprize.org/play?task=0dfd9992
p=lambda g:[g:=[*zip(*[max(a*all(len({*r,0})<3for r in zip(b,a))for a in g)for b in g])]for _ in g][1]
# 💎 task 18: 275 (402 unzipped) bytes, gold, https://arcprize.org/play?task=0e206a2e
...
# 🥈 task 19: 104 vs 103 bytes for gold, https://arcprize.org/play?task=10fcaaa3
*z,p=[0]*9,eval('lambda a:[[a[1][1]or 8*any(sum(a,())[::2])'+'for*a,in map(zip,z+a+a,a+a,a[1:]+a+z)]'*2)
# ❌ task 20: 139 vs 126 bytes for gold, https://arcprize.org/play?task=11852cab
...
# 💎 task 23: 182 bytes, gold, https://arcprize.org/play?task=150deff5
...
# 🥈 task 25: 134 vs 127 bytes for gold, https://arcprize.org/play?task=1a07d186
p=lambda g:[[sum({v}&{*r[:j]}|{*r[j:]}&{m[j],m[j:=j+1]})for v in m[2:]]for r in g if any(m:=[0,*min(g),j:=0])]or[*zip(*p([*zip(*g)]))]
# ❌ task 26: 52 vs 50 bytes for gold, https://arcprize.org/play?task=1b2d62fb
p=lambda a:[[8>>i+b.pop(0)for i in b[4:]]for b in a]
# 🥈 task 27: 103 vs 97 bytes for gold, https://arcprize.org/play?task=1b60fb0c
...
# ❌ task 29: 110 vs 103 bytes for gold, https://arcprize.org/play?task=1c786137
exec("p=lambda a,d=1:[i:="+'[b[1:-1]for*b,in zip(*'*2+"a)if d in b])if{d}<{*b}],0][d in sum(i,a)]or p(a,d+1)")
# ❌ task 30: 94 vs 93 bytes for gold, https://arcprize.org/play?task=1caeab9d
...
# 🥈 task 33: 73 vs 71 bytes for gold, https://arcprize.org/play?task=1e32b0e9
p=lambda g,t=[],s=0:g*0!=0and[*map(p,g,(t+g)[:6]*3,[g[5]]*17)]or(t>g)*s|g
# 🥈 task 34: 125 vs 116 bytes for gold, https://arcprize.org/play?task=1f0c79e5
import re;p=lambda i:exec(r'i[::-1]=zip(*eval(re.sub("(?=(.{32})*(., |.{29})?2, [^0].{25}([^02]))\d","\\3",str(i))));'*8)or i
# 🥇 task 35: 82 bytes, gold, https://arcprize.org/play?task=1f642eb9
p=lambda i,k=3:-k*i or[[k:=i[y!=8<<k]or y for y in i]for i[::-1]in zip(*p(i,k-1))]
# ❌ task 36: 82 vs 75 bytes for gold, https://arcprize.org/play?task=1f85a75f
p=lambda i,n=2,*t:t*(t==t[:5])or p(i,n+1,*zip(*[x for x in[i,t][n%2]if n//2in x]))
# 🥈 task 37: 104 vs 101 bytes for gold, https://arcprize.org/play?task=1f876c06
...
# 🥈 task 40: 69 vs 63 bytes for gold, https://arcprize.org/play?task=2204b7a8
...
# 🥈 task 42: 134 vs 132 bytes for gold, https://arcprize.org/play?task=22233c11
...
# 🥈 task 46: 168 vs 166 bytes for gold, https://arcprize.org/play?task=234bbc79
...
# 🥈 task 50: 84 vs 83 bytes for gold, https://arcprize.org/play?task=253bf280
...
p=lambda a:(a+a)[2:5]
# 🥈 task 54: 270 (372 unzipped) vs 266 bytes for gold, https://arcprize.org/play?task=264363fd
def	p(s):
...
# 🥈 task 56: 40 vs 39 bytes for gold, https://arcprize.org/play?task=27a28665
...
# 💎 task 58: 85 bytes, gold, https://arcprize.org/play?task=28e73c20
p=lambda a:a and[len(a)*[3],[*a[0],3>>2%len(a),3][2:],*zip(*p([*zip(*a[2:])])[::-1])]
# 🥈 task 59: 153 vs 151 bytes for gold, https://arcprize.org/play?task=29623171
...
# ❌ task 61: 63 vs 62 bytes for gold, https://arcprize.org/play?task=29ec7d0e
...
# 🥉 task 64: 152 vs 135 bytes for gold, https://arcprize.org/play?task=2c608aff
...
# 🥈 task 65: 84 vs 82 bytes for gold, https://arcprize.org/play?task=2dc579da
p=lambda a:[p(b)for*b,in map(zip,a,a[len(a)//2+1:])]or min(b:=sum(a,()),key=b.count)
# 🥉 task 66: 244 (319 unzipped) vs 237 bytes for gold, https://arcprize.org/play?task=2dd70a9a
def p(n):
...
# 🥈 task 68: 103 vs 99 bytes for gold, https://arcprize.org/play?task=31aa019c
p=lambda a,k=6,q=0:~k*a or p([[[2*q,v][q:=sum(a,r).count(v)<9-k]for v in r]for*r,in zip(*a)][::-1],k-2)
# 🥉 task 69: 150 vs 143 bytes for gold, https://arcprize.org/play?task=321b1fc6
...
# 🥈 task 71: 104 vs 100 bytes for gold, https://arcprize.org/play?task=3345333e
...
# 🥉 task 76: 269 (384 unzipped) vs 258 bytes for gold, https://arcprize.org/play?task=36d67576
def	p(f):
...
# 🥈 task 81: 83 vs 81 bytes for gold, https://arcprize.org/play?task=3aa6fb7a
...
# 🥉 task 88: 100 vs 95 bytes for gold, https://arcprize.org/play?task=3de23699
...
def	p(i):
	...
	return	i
# 🥈 task 90: 153 vs 147 bytes for gold, https://arcprize.org/play?task=3eda0437
s='for %s,b in enumerate(%s)'
exec(f"p=lambda a:max([j>i,-(C:=sum(g:=[[b|6*(i<=m<=j)*(k<=n<=l){s}]{s}],a).count)(7),C(6),g]{s*4})[3]"%(*'nbmaiakbjalb',))
# 🥇 task 91: 62 bytes, gold, https://arcprize.org/play?task=3f7978a0
...
# 🥇 task 93: 98 bytes, gold, https://arcprize.org/play?task=4093f84a
import re;p=lambda g:exec(r'g[::-1]=zip(*eval(re.sub("[^50],([^[(]+5)",r"\1,5",str(g))));'*12)or g
# 🥈 task 94: 97 vs 96 bytes for gold, https://arcprize.org/play?task=41e4d17e
p=lambda i,*h:[h:=[y+6%-y*(1<x.count(1)==h.count(h>[7])%5)for y in x]for x in zip(*h or p(i,*i))]
# 🥉 task 95: 74 vs 70 bytes for gold, https://arcprize.org/play?task=4258a5f9
p=lambda a,n=6:~n*a or[[n%2|(n:=m)for m in a]for a[::-1]in zip(*p(a,n-2))]
# 🥇 task 96: 282 (377 unzipped) bytes, gold, https://arcprize.org/play?task=4290ef0e
import re
...
# 🥈 task 99: 103 vs 93 bytes for gold, https://arcprize.org/play?task=444801d8
...
# 💎 task 101: 258 (431 unzipped) bytes, gold, https://arcprize.org/play?task=447fd412
def p(e):
 ...
 return e
# 🥉 task 102: 135 vs 124 bytes for gold, https://arcprize.org/play?task=44d8ac46
...
# 🥇 task 104: 84 bytes, gold, https://arcprize.org/play?task=4522001f
p=lambda g,h=2,w=3:h and[p(g,h-1,w&2-A//4)for A in range(9)][::1-g[h][-h]|1]or-w%3^w
# ❌ task 105: 139 vs 124 bytes for gold, https://arcprize.org/play?task=4612dd53
...
# 🥈 task 110: 84 vs 83 bytes for gold, https://arcprize.org/play?task=484b58aa
...
# ❌ task 112: 106 vs 91 bytes for gold, https://arcprize.org/play?task=4938f0c2
...
# 🥉 task 115: 52 vs 51 bytes for gold, https://arcprize.org/play?task=4be741c5
p=lambda a,*n:[*zip(*map({}.fromkeys,n or p(a,*a)))]
# 🥇 task 116: 20 bytes, gold, https://arcprize.org/play?task=4c4377d9
p=lambda a:a[::-1]+a
# 🥈 task 117: 130 vs 115 bytes for gold, https://arcprize.org/play?task=4c5c2cf0
...
# 🥉 task 121: 88 vs 85 bytes for gold, https://arcprize.org/play?task=5117e062
p=lambda g:exec("g[:]=map(list,zip(*g[(8in g[-2])-2::-1]));"*40+"g[1][1]=max(g[0])")or g
# 🥈 task 122: 71 vs 70 bytes for gold, https://arcprize.org/play?task=5168d44c
p=lambda a:'3, 0'in'%s'%max(a)and[*map(p,a)]or a[:len(a)^1][-2:]+a[:-2]
# ❌ task 123: 75 vs 71 bytes for gold, https://arcprize.org/play?task=539a4f51
R=range(10)
...
# 🥈 task 125: 116 vs 110 bytes for gold, https://arcprize.org/play?task=543a7ed5
...
# 🥉 task 127: 63 vs 57 bytes for gold, https://arcprize.org/play?task=54d9e175
p=lambda a,q=3:a*-1and 5+a%5or a[1:]and[p(a[1])]*q+p(a[2:],4-q)
# 🥈 task 128: 59 vs 57 bytes for gold, https://arcprize.org/play?task=5521c0d9
...
# 🥈 task 130: 65 vs 61 bytes for gold, https://arcprize.org/play?task=5614dbcf
p=lambda a:[[sum({*b[c:c+3]}-{5})for c in(0,3,6)]for b in a[::3]]
# 🥈 task 131: 125 vs 117 bytes for gold, https://arcprize.org/play?task=56dc2b01
...
# 🥈 task 136: 97 vs 94 bytes for gold, https://arcprize.org/play?task=5c0a986e
...
# ❌ task 138: 104 vs 103 bytes for gold, https://arcprize.org/play?task=5daaa586
p=lambda a,n=35:-n*a or p([*map(lambda*b,d=0:[d:=c|d*(d==b[0]>n-3)for c in b[::-1]],*a[0in a[0]:])],n-1)
# 🥈 task 139: 88 vs 81 bytes for gold, https://arcprize.org/play?task=60b61512
...
p=lambda a:[b[::-1]for b in a[::-1]]
# 🥉 task 141: 93 vs 92 bytes for gold, https://arcprize.org/play?task=623ea044
...
p=lambda a:[b+b[::-1]for b in a+a[::-1]]
# 🥈 task 143: 130 vs 127 bytes for gold, https://arcprize.org/play?task=63613498
...
# 🥈 task 148: 124 vs 118 bytes for gold, https://arcprize.org/play?task=673ef223
p=lambda g,*w:[[-v%12&6|(c:=c^(8in r*v))*8>>v for v in r]for r in g if[c:=any(r)and(w:=[r,*w])[0][0]!=g[4][0]!=8in w.pop()]]
# 🥈 task 149: 75 vs 74 bytes for gold, https://arcprize.org/play?task=6773b310
...
# 🥉 task 151: 103 vs 100 bytes for gold, https://arcprize.org/play?task=67a423a3
def p(g):
 y,x,*_=[z.index(max(z))for z in[g]+g]
 for N in b"":g[y+N//3-6][x+N%3-1]=4
 return g
# 🥈 task 152: 40 vs 39 bytes for gold, https://arcprize.org/play?task=67e8384a
p=lambda a:[r+r[::-1]for r in a+a[::-1]]
# ❌ task 153: 131 vs 118 bytes for gold, https://arcprize.org/play?task=681b3aeb
...
p=lambda a:a[::-1]
# 🥈 task 156: 128 vs 118 bytes for gold, https://arcprize.org/play?task=694f12f3
p=lambda i,a=9,S=sum:[[v-(S(g:=S(i,[]))//S(g[:i.index(min(i),1)*10]*2)+2^a//60)*(S(g[a-18:(a:=a+1):9])>11)for v in r]for r in i]
# ❌ task 157: 242 (256 unzipped) vs 237 bytes for gold, https://arcprize.org/play?task=6a1e5592
def	p(g):
		...
		for	f	in	range(len(r)-e*3):
			for	o	in	range(len(r[-1])-e*3):
				for	a	in	range(len(r[-1])):
					...
					l=*zip(*l[::-1]),
	return	r
# 🥈 task 159: 105 vs 103 bytes for gold, https://arcprize.org/play?task=6b9890af
...
# 🥇 task 163: 130 bytes, gold, https://arcprize.org/play?task=6d0160f0
...
# ❌ task 165: 130 vs 112 bytes for gold, https://arcprize.org/play?task=6d58a25d
...
# ❌ task 168: 112 vs 110 bytes for gold, https://arcprize.org/play?task=6e19193c
import re;p=lambda i:exec(r'i[::-1]=zip(*eval(re.sub(r"0(?=(.{35})+,( [^0]).{27}\2,\2)",r"\2",str(i))));'*4)or i
# 🥉 task 169: 111 vs 108 bytes for gold, https://arcprize.org/play?task=6e82a1ae
p=lambda i,k=11,t=1:-k*i or p([(e:=1)*[e:=y%2*[y|(t:=t*16)|e,-y%5][k<1]for y in i]for i[::-1]in zip(*i)],k-1,0)
# ❌ task 170: 193 vs 163 bytes for gold, https://arcprize.org/play?task=6ecd11f4
...
p=lambda a:a+a[::-1]
# 🥈 task 173: 204 (299 unzipped) vs 201 bytes for gold, https://arcprize.org/play?task=72322fa7
...
# 🥈 task 175: 75 vs 74 bytes for gold, https://arcprize.org/play?task=73251a56
...
def p(a):
	...
	return a
# ❌ task 182: 186 vs 160 bytes for gold, https://arcprize.org/play?task=776ffc46
def p(g):
 R=F=sum(g,[])
 while C:=max(F):
  ...
  if~-C:R,W=N,C
 return g
# ❌ task 183: 94 vs 79 bytes for gold, https://arcprize.org/play?task=77fdfe62
def p(g):A=len(g);B=range(2,A-2);return[[g[g[C][B]%5-3-C*2//A][0-B*2//A]for B in B]for C in B]
# ❌ task 184: 94 vs 91 bytes for gold, https://arcprize.org/play?task=780d0b14
p=lambda i,k=0,*s:[s+(s:=())for x in zip(*k or p(i,i))if-~-any(x)*(s:=(*map(max,s+x,x),))]+[s]
# ❌ task 185: 121 vs 107 bytes for gold, https://arcprize.org/play?task=7837ac64
...
# 🥈 task 187: 88 vs 83 bytes for gold, https://arcprize.org/play?task=7b6016b9
p=lambda i,k=7:-k*i or[*map(lambda*x,z=3:[z:=y|(y*z==6)or 2for y in x],*p(i,k-1)[::-1])]
# ❌ task 188: 63 vs 53 bytes for gold, https://arcprize.org/play?task=7b7f7511
p=lambda a,s=4:a[:s]*(a[:s]*2==a)or[*zip(*p([*zip(*a)],~-s%5))]
# 🥈 task 189: 111 vs 95 bytes for gold, https://arcprize.org/play?task=7c008303
p=lambda i,r=range(6):[[i[a-6+(s:=i[6][0]%3)*3][b-6+(t:=i[0][6]%3)*3]/3*i[a//3-s][b//3-t]for b in r]for a in r]
# 💎 task 190: 103 bytes, gold, https://arcprize.org/play?task=7ddcd7ec
import re;p=lambda i:exec(r"i[::-1]=zip(*eval(re.sub('.{31}0, ([^0])'*2,r'|\1\g<0>',str(i))));"*20)or i
# 🥈 task 191: 227 (384 unzipped) vs 221 bytes for gold, https://arcprize.org/play?task=7df24a62
def	p(a):
	...
	return	a
# ❌ task 192: 114 vs 99 bytes for gold, https://arcprize.org/play?task=7e0986d6
...
# 🥈 task 196: 106 vs 105 bytes for gold, https://arcprize.org/play?task=810b9b61
p=lambda i,k=19:-k*i or[*map(lambda*r,a=8:[a:=[b|8-b&a,b|2*(a%3<b),b%5][k//8]for b in r],*p(i,k-1)[::-1])]
# 🥉 task 197: 54 vs 51 bytes for gold, https://arcprize.org/play?task=82819916
p=lambda a:[[b[a[1].index(c)]for c in a[1]]for b in a]
# ❌ task 198: 118 vs 107 bytes for gold, https://arcprize.org/play?task=83302e8f
p=lambda a,*b,n=-23:n*a or[[c,4-(sum(25>>e&n for e in b)>9)][9>>c&(n:=c!=4)]for c in b][::-1]or[*map(p,a,*p(a,n=n+1))]
# ❌ task 199: 84 vs 77 bytes for gold, https://arcprize.org/play?task=834ec97d
...
# 🥈 task 201: 188 vs 187 bytes for gold, https://arcprize.org/play?task=846bdb03
...
# ❌ task 202: 104 vs 98 bytes for gold, https://arcprize.org/play?task=855e0971
...
# 🥈 task 205: 136 vs 134 bytes for gold, https://arcprize.org/play?task=8731374e
p=lambda a,n=91:-n*a or p([[[min(b+c,key=c.count)for*c,in a],A:=b][n>0]for*b,in zip(*a)][any(b'%c'%x*6in bytes(A)for x in A)-2::-1],n-1)
# 🥈 task 206: 140 vs 139 bytes for gold, https://arcprize.org/play?task=88a10436
def p(g):
...
# 🥉 task 208: 211 (270 unzipped) vs 163 bytes for gold, https://arcprize.org/play?task=890034e9
def	p(z):
...
# ❌ task 213: 90 vs 86 bytes for gold, https://arcprize.org/play?task=8e1813be
...
# 🥈 task 216: 113 vs 111 bytes for gold, https://arcprize.org/play?task=8efcae92
...
# 🥉 task 219: 242 (261 unzipped) vs 241 bytes for gold, https://arcprize.org/play?task=90f3ed37
def p(o):
 e=n=i=0
 while o[i:]:
  ...
  i+=1
 return o
# 🥉 task 220: 88 vs 82 bytes for gold, https://arcprize.org/play?task=913fb3ed
...
# 🥈 task 224: 131 vs 114 bytes for gold, https://arcprize.org/play?task=928ad970
p=lambda g,k=6,w=0:~k*g or[[a+[(0is a<w<=k)*max({*sum(g,r)}-{5}),0.][2>w]for a in r]for*r,in zip(*p(g,k-1))if[w:=w*2+any(r)]][::-1]
# ❌ task 225: 126 vs 123 bytes for gold, https://arcprize.org/play?task=93b581b8
p=eval(f'lambda i:[[([i[a][b]]*4+[i{" for i in i[a-3:a]+i[a+3:a:-1]for a in[b]"*2}if i])[-4]'+'for %c in range(6)]'*2%(98,97))
# 🥈 task 226: 119 vs 117 bytes for gold, https://arcprize.org/play?task=941d9a10
...
# 🥇 task 228: 116 bytes, gold, https://arcprize.org/play?task=952a094c
import re;p=lambda a:exec(r"a[::-1]=zip(*eval(re.sub(r'([^0])((, (?!\1|0).).*0\3.{28})0',r'0\2\1',str(a))));"*4)or a
...
# 🥉 task 230: 110 vs 95 bytes for gold, https://arcprize.org/play?task=95990924
import re;p=lambda m:[m:=[*zip(*eval(re.sub("0(?=, 0.%s.5, 5)"%{len(m)*3},i,str(m)))[::-1])]for i in'1342'][3]
# 🥈 task 231: 43 vs 42 bytes for gold, https://arcprize.org/play?task=963e52fc
...
# 🥈 task 233: 271 (440 unzipped) vs 261 bytes for gold, https://arcprize.org/play?task=97a05b5b
def p(r):
 ...
 return r
# 🥈 task 234: 107 vs 106 bytes for gold, https://arcprize.org/play?task=98cf29f8
...
# ❌ task 237: 67 vs 66 bytes for gold, https://arcprize.org/play?task=99fa7670
p=lambda a,c=0:[(b:=0)or[c:=(b:=b or d)for d in r+[c]]for*r,_ in a]
# 🥈 task 238: 201 (313 unzipped) vs 195 bytes for gold, https://arcprize.org/play?task=9aec4887
...
# ❌ task 240: 104 vs 89 bytes for gold, https://arcprize.org/play?task=9d9215db
...
# 🥈 task 244: 63 vs 61 bytes for gold, https://arcprize.org/play?task=9f236235
p=lambda a:[b[::(n:=~a.index(min(a,key=set)))]for b in a][::-n]
# 🥇 task 245: 98 bytes, gold, https://arcprize.org/play?task=a1570a43
p=lambda i,k=7:-k*i or p([*map(lambda*r,w=0:[[w%3,w:=v][v%2+5>sum({*max(i)})]for v in r],*i)],k-1)
# 🥈 task 246: 105 vs 104 bytes for gold, https://arcprize.org/play?task=a2fd1cf0
...
# ❌ task 248: 74 vs 67 bytes for gold, https://arcprize.org/play?task=a3df8b1e
def p(m,c=0,d=1):
 for r in m[::-1]:r[c]=1;c+=d;r[1:-c]or(d:=-d)
 return m
# 🥇 task 249: 26 bytes, gold, https://arcprize.org/play?task=a416b8f3
p=lambda a:[r*2for r in a]
# 💎 task 250: 95 bytes, gold, https://arcprize.org/play?task=a48eeaf7
p=lambda a:[a:=[sorted(b[:(i:=str(a).find('2')>>5)])+b[i:]for*b,in zip(*a)][::-1]for _ in a][3]
# ❌ task 251: 89 vs 84 bytes for gold, https://arcprize.org/play?task=a5313dff
...
# ❌ task 254: 91 vs 81 bytes for gold, https://arcprize.org/play?task=a61f2674
p=lambda i,r=[0]*9:[r:=[y or s>0and((x<i[8])is y)*2+1-any(r)for s,y in zip(x,r)]for x in i]
# 💎 task 255: 212 bytes, gold, https://arcprize.org/play?task=a64e4611
...
# 🥉 task 256: 96 vs 93 bytes for gold, https://arcprize.org/play?task=a65b410d
...
# 🥉 task 259: 85 vs 83 bytes for gold, https://arcprize.org/play?task=a740d043
...
# ❌ task 263: 117 vs 106 bytes for gold, https://arcprize.org/play?task=a87f7484
p=lambda g:g[(T:=[*zip(*[map(bool,sum(g,g*0))]*9)]).index(min(T,key=T.count))*3:][:3%len(g)]or[*zip(*p((*zip(*g),)))]
# ❌ task 264: 190 (310 unzipped) vs 178 bytes for gold, https://arcprize.org/play?task=a8c38be5
...
# ❌ task 266: 99 vs 88 bytes for gold, https://arcprize.org/play?task=a9f96cdd
...
# ❌ task 268: 222 (278 unzipped) vs 190 bytes for gold, https://arcprize.org/play?task=aba27056
...
# ❌ task 270: 117 vs 115 bytes for gold, https://arcprize.org/play?task=ae3edfdc
...
# 🥈 task 273: 81 vs 75 bytes for gold, https://arcprize.org/play?task=af902bf9
p=lambda a,*A,d=0:[[c^(2%-~c|(d:=2&d-c%3))for c in b]for b in zip(*A or p(a,*a))]
# ❌ task 274: 71 vs 65 bytes for gold, https://arcprize.org/play?task=b0c4d837
p=lambda i:[(h:=[8for x in i if sum(x)==10]+[0]*5)[:3],h[5:2:-1],[0]*3]
# 🥈 task 275: 130 vs 126 bytes for gold, https://arcprize.org/play?task=b190f7f5
p=lambda i:('8'in str(j:=i[(u:=len(i[0])):]))*[[t*y/8for t in s for y in x]for s in i[:u]for x in j]or[*zip(*p([*zip(*j+i[:u])]))]
# 🥈 task 276: 38 vs 37 bytes for gold, https://arcprize.org/play?task=b1948b0a
...
# 🥉 task 280: 168 vs 145 bytes for gold, https://arcprize.org/play?task=b527c5c6
def p(a,n=3,i=0):
 for b in a:
...
# 🥈 task 282: 76 vs 74 bytes for gold, https://arcprize.org/play?task=b60334d2
...
# 🥈 task 284: 204 vs 195 bytes for gold, https://arcprize.org/play?task=b7249182
...
# ❌ task 285: 272 (431 unzipped) vs 226 bytes for gold, https://arcprize.org/play?task=b775ac94
def	p(f):
	for*i,in[[]]*8:
...
# 🥈 task 287: 55 vs 53 bytes for gold, https://arcprize.org/play?task=b8825c91
p=lambda*a:sum({*a[3:]}-{4})or[*map(p,*a+a,a[0][::-1])]
# ❌ task 288: 91 vs 86 bytes for gold, https://arcprize.org/play?task=b8cdaf2b
def p(i,n=0):
 l=len(i)//2;*w,S,L=i
...
p=lambda a:eval("[[a\nfor a in a for _ in[*{*'%s'}][5:]]#"%a*2)
# 🥉 task 290: 68 vs 65 bytes for gold, https://arcprize.org/play?task=b94a9452
...
# ❌ task 296: 62 vs 55 bytes for gold, https://arcprize.org/play?task=bc1d5164
...
# 🥈 task 298: 55 vs 54 bytes for gold, https://arcprize.org/play?task=bda2d7a6
...
# 🥈 task 305: 57 vs 56 bytes for gold, https://arcprize.org/play?task=c3f564a4
...
# ❌ task 308: 205 vs 192 bytes for gold, https://arcprize.org/play?task=c8cbb738
e=enumerate
...
# 🥈 task 309: 38 vs 36 bytes for gold, https://arcprize.org/play?task=c8f0f002
...
# 🥈 task 313: 62 vs 61 bytes for gold, https://arcprize.org/play?task=caa06a1f
...
# 🥈 task 320: 66 vs 65 bytes for gold, https://arcprize.org/play?task=ce9e57f2
p=lambda a:[*zip(*[c[:-(w:=sum(c)//4)]+w*[8]+c for*c,in zip(*a)])]
# ❌ task 321: 55 vs 54 bytes for gold, https://arcprize.org/play?task=cf98881b
p=lambda m:[[r.pop(0)or r[4]|r[9]for _ in m]for r in m]
# 🥉 task 322: 48 vs 45 bytes for gold, https://arcprize.org/play?task=d037b0a7
p=lambda a:[[*map(max,*a[:n]*2)]for n in(1,2,3)]
# 🥈 task 323: 104 vs 100 bytes for gold, https://arcprize.org/play?task=d06dbe63
p=lambda a,B=0:[[v|5*((b:=abs(B+(B:=B-3)//39*2+str(a).find('8')))>~b%76in b'KHE"')for v in r]for r in a]
# ❌ task 324: 227 (261 unzipped) vs 216 bytes for gold, https://arcprize.org/play?task=d07ae81c
...
# 🥉 task 331: 83 vs 82 bytes for gold, https://arcprize.org/play?task=d364b489
...
# 🥈 task 333: 89 vs 83 bytes for gold, https://arcprize.org/play?task=d43fd935
p=lambda a,n=-3:n*a or[(d:=0)or[d:=b.pop()or d*(3in b)for _ in a]for*b,in zip(*p(a,n+1))]
# 🥉 task 334: 69 vs 65 bytes for gold, https://arcprize.org/play?task=d4469b4b
p=lambda i:[[(y>>max(max(i)))%2*5for y in[x,x|6,x|8]]for x in[4,2,8]]
# 🥉 task 335: 110 vs 105 bytes for gold, https://arcprize.org/play?task=d4a91cb9
p=lambda a,n=10,d=0:~n*a or p([[b.pop()|(n%6*2in b)*(d:=d^4%(sum(c)|4))for c in a[::-1]]for*b,in zip(*a)],n-3)
# 🥉 task 336: 90 vs 86 bytes for gold, https://arcprize.org/play?task=d4f3cd78
...
# ❌ task 338: 65 vs 62 bytes for gold, https://arcprize.org/play?task=d5d6de2d
...
# 🥈 task 340: 112 vs 111 bytes for gold, https://arcprize.org/play?task=d687bc17
p=lambda i:[i:=[[sum({v,r[0]}&{*i[1],*r[~(j:=j%5%-3):]})for v in r][::-1]for*r,in zip(*i)if[j:=2]]for _ in i][3]
# 🥉 task 341: 115 vs 108 bytes for gold, https://arcprize.org/play?task=d6ad076f
p=lambda i:[i:=[[b[a]or(v:=2+({*s}>{*s[a:]}>{0}and v))&8for b,s in zip(i,i[1:]+i)]for a in range(10)]for r in i][1]
# 🥉 task 342: 108 vs 106 bytes for gold, https://arcprize.org/play?task=d89b689b
import re;p=lambda i:exec(r'i[::-1]=zip(*eval(re.sub("([1-9])((.{32})+?[0, ]+)8",r"0\2\1",str(i))));'*4)or i
# 🥈 task 343: 64 vs 63 bytes for gold, https://arcprize.org/play?task=d8c310e9
...
# 🥉 task 348: 94 vs 88 bytes for gold, https://arcprize.org/play?task=db3e9e38
p=lambda a,n=-13:n*a or p([[c|-d%15for c,d in zip(a.pop(0),[0]+b)][::-1]for*b,in a[1:]]+a,n+1)
# ❌ task 349: 203 (277 unzipped) vs 172 bytes for gold, https://arcprize.org/play?task=db93a21d
...
# 🥉 task 354: 96 vs 92 bytes for gold, https://arcprize.org/play?task=ddf7fa4f
p=lambda i:[i:=[[(y==5)*c[c[1]>0]or y for*c,y in zip([0]+x,*i,x)][::-1]for x in i]for x in i][9]
# 🥉 task 355: 101 vs 95 bytes for gold, https://arcprize.org/play?task=de1cd16c
p=lambda a:[sorted(range(10),key=lambda c:sum(e!=c in{*b}&{*d}for b in a for*d,e in zip(*a,b)))[8:9]]
# 🥈 task 356: 100 vs 92 bytes for gold, https://arcprize.org/play?task=ded97339
p=lambda i,*I:[[W[b]|sum({*w[b:]}&{*w[:b]})for b in range(10)]for*W,w in zip(*I or p(zip(*i),*i),i)]
# ❌ task 357: 86 vs 81 bytes for gold, https://arcprize.org/play?task=e179c5f4
p=lambda m,x=1,d=1:m and p(m,x:=x-d*2+1,1>>x*(l:=len(m.pop())+~x)^d)+[[8]*x+[1]+[8]*l]
# 🥈 task 358: 91 vs 90 bytes for gold, https://arcprize.org/play?task=e21d9049
...
# 🥉 task 362: 69 vs 67 bytes for gold, https://arcprize.org/play?task=e48d4e1a
p=lambda g:[r[(n:=g.count(g[0])):9]+r[:1]+r[:n]for r in g*2][10-n:-n]
# 🥉 task 363: 204 vs 175 bytes for gold, https://arcprize.org/play?task=e5062a87
def p(g):
 ...
 return g
# 🥈 task 364: 134 vs 133 bytes for gold, https://arcprize.org/play?task=e509e548
...
# ❌ task 370: 227 (357 unzipped) vs 159 bytes for gold, https://arcprize.org/play?task=e8dc4411
def	p(r):
	...
	return	r
# 🥈 task 371: 109 vs 102 bytes for gold, https://arcprize.org/play?task=e9614598
def p(a):
 for i in[l:=len(a[0]),-l,1,0,-1]:b=sum(a,[]).index;i+=b(1,b(1)+1)+b(1)>>1;a[i//l][i%l]=3
 return a
# ❌ task 372: 48 vs 47 bytes for gold, https://arcprize.org/play?task=e98196ab
...
# ❌ task 374: 107 vs 103 bytes for gold, https://arcprize.org/play?task=ea32f347
...
# 🥈 task 378: 138 vs 131 bytes for gold, https://arcprize.org/play?task=ec883f72
import re;p=lambda i:exec(r"i[::-1]=zip(*eval(re.sub('0(?=(%s 0)*%s ., [^0]%s?%s (.))'%(('.%s.0,'%{3*len(i)},)*4),'\\2',str(i))));"*4)or i
# ❌ task 379: 141 vs 135 bytes for gold, https://arcprize.org/play?task=ecdecbb3
import re;p=lambda i,r=re.sub:exec(r'i[:]=zip(*eval(r(", 4, ","|8,8,8|",r("0, 8, ((0, )+)2","4,2,4,*[2]*len([\\1])",str(i[::-1])))));'*8)or i
...
# 🥈 task 383: 121 vs 120 bytes for gold, https://arcprize.org/play?task=f1cefba8
p=lambda g,*G:[[r,[(o:=[*{}.fromkeys(r)]*3)[1+0**v]for v in r]][str(o)[1:8]in f'{r+r[::-1]}']for r in zip(*G or p(g,*g))]
# 🥈 task 384: 62 vs 61 bytes for gold, https://arcprize.org/play?task=f25fbde4
...
# ❌ task 386: 52 vs 50 bytes for gold, https://arcprize.org/play?task=f2829549
p=lambda a:[[3>>e+r.pop(0)for e in r[4:]]for r in a]
# 🥉 task 387: 205 vs 192 bytes for gold, https://arcprize.org/play?task=f35d900a
...
# ❌ task 392: 152 vs 135 bytes for gold, https://arcprize.org/play?task=f8c80d96
...
# 🥈 task 397: 113 vs 110 bytes for gold, https://arcprize.org/play?task=fcc82909
p=lambda m,k=71:exec('r=k//9;exec("%s=3,3;"*len(a:={*%s+%s})*all(a));k-=1;'%(('m[r:=r+1][k%9:k%9+2]',)*3)*81)or m
# 🥉 task 398: 73 vs 72 bytes for gold, https://arcprize.org/play?task=feca6190
p=lambda a,b=45:[*zip(*[2*((b:=b-1)*[0]+a[0])for c in 5*a[0]if c])][b:45]
# 🥈 task 399: 63 vs 62 bytes for gold, https://arcprize.org/play?task=ff28f65a
