...
# task 4: 85 vs 80 bytes for gold, https://arcprize.org/play?task=025d127b
import re
...
# task 7: 67 vs 63 bytes for gold, https://arcprize.org/play?task=05269061
p=lambda a,*b:[[max(sum(a,b:=[*b,0,0])[2::3])for _ in a]for _ in a]
# task 8: 94 vs 85 bytes for gold, https://arcprize.org/play?task=05f2a901
...
# task 12: 133 vs 127 bytes for gold, https://arcprize.org/play?task=0962bcdd
...
# task 23: 206 vs 195 bytes for gold, https://arcprize.org/play?task=150deff5
import re
...
# task 33: 77 vs 73 bytes for gold, https://arcprize.org/play?task=1e32b0e9
p=lambda g:[[v+(v<V)*R[5]for v,V in zip(r,R[:6]*3)]for r,R in zip(g,g[:6]*3)]
# task 34: 178 vs 125 bytes for gold, https://arcprize.org/play?task=1f0c79e5
...
# task 42: 169 vs 143 bytes for gold, https://arcprize.org/play?task=22233c11
Q=range(10)
...
# task 44: 281 (376 unzipped) vs 279 bytes for gold, https://arcprize.org/play?task=228f6490
def p(i):
 for n in{*sum(i,[])}:
...
# task 50: 91 vs 86 bytes for gold, https://arcprize.org/play?task=253bf280
p=lambda a:[*map(f:=lambda*b,i=0:[c|3*(8in{*b[:i]}&{*b[(i:=i+1):]})for c in b],*map(f,*a))]
# task 51: 123 vs 115 bytes for gold, https://arcprize.org/play?task=25d487eb
import re
...
# task 69: 209 vs 151 bytes for gold, https://arcprize.org/play?task=321b1fc6
def p(a,i=-1):
...
# task 76: 302 (355 unzipped) bytes, gold, https://arcprize.org/play?task=36d67576
E=enumerate
def p(g):
...
# task 92: 93 vs 87 bytes for gold, https://arcprize.org/play?task=40853293
p=lambda a:[*map(f:=lambda*b,i=-1:[b[i:=i+1]or sum({*b[i:]}&{*b[:i]})for _ in b],*map(f,*a))]
# task 93: 102 vs 99 bytes for gold, https://arcprize.org/play?task=4093f84a
import re
...
# task 95: 79 vs 73 bytes for gold, https://arcprize.org/play?task=4258a5f9
...
# task 99: 150 vs 115 bytes for gold, https://arcprize.org/play?task=444801d8
...
# task 101: 281 (338 unzipped) bytes, gold, https://arcprize.org/play?task=447fd412
def p(g):
...
# task 118: 302 (384 unzipped) vs 279 bytes for gold, https://arcprize.org/play?task=50846271
def p(g,F=0):
 for W in 4,4,3:
...
# task 122: 94 vs 82 bytes for gold, https://arcprize.org/play?task=5168d44c
...
# task 124: 129 vs 97 bytes for gold, https://arcprize.org/play?task=53b68214
...
# task 137: 141 vs 143 bytes for gold, https://arcprize.org/play?task=5c2c9af4
...
# task 157: 328 (403 unzipped) vs 291 bytes for gold, https://arcprize.org/play?task=6a1e5592
def p(g):
 def Q(o,p):
  c=C[o:];(3,)<=c!=Q(o+1,p)
  for A in S:
...
def p(g):
 ...
 return g
# task 159: 110 bytes, gold, https://arcprize.org/play?task=6b9890af
...
# task 162: 102 vs 96 bytes for gold, https://arcprize.org/play?task=6cf79266
...
p=lambda a:a+a[::-1]
# task 173: 228 (271 unzipped) vs 218 bytes for gold, https://arcprize.org/play?task=72322fa7
...
# task 187: 107 vs 94 bytes for gold, https://arcprize.org/play?task=7b6016b9
...
# task 193: 98 vs 81 bytes for gold, https://arcprize.org/play?task=7f4411dc
import re
...
# task 219: 292 (307 unzipped) vs 267 bytes for gold, https://arcprize.org/play?task=90f3ed37
Z=zip
def p(a,h=0):
...
# task 220: 95 vs 89 bytes for gold, https://arcprize.org/play?task=913fb3ed
...
# task 222: 105 vs 106 bytes for gold, https://arcprize.org/play?task=91714a58
...
# task 244: 64 vs 65 bytes for gold, https://arcprize.org/play?task=9f236235
p=lambda a,n=1:[b[::~n]for b in(a[n]!=a[0])*a][::1+n]or p(a,n+1)
# task 245: 117 vs 110 bytes for gold, https://arcprize.org/play?task=a1570a43
...
# task 250: 127 vs 123 bytes for gold, https://arcprize.org/play?task=a48eeaf7
import re
...
def p(i):
 ...
 return i
# task 285: 302 (426 unzipped) vs 291 bytes for gold, https://arcprize.org/play?task=b775ac94
def p(g):
 for E in range(8):
  I=[]
...
# task 308: 245 (253 unzipped) vs 238 bytes for gold, https://arcprize.org/play?task=c8cbb738
def p(g):
...
# task 319: 243 (293 unzipped) vs 194 bytes for gold, https://arcprize.org/play?task=ce602527
...
# task 323: 153 vs 102 bytes for gold, https://arcprize.org/play?task=d06dbe63
...
# task 328: 188 vs 167 bytes for gold, https://arcprize.org/play?task=d22278a0
...
# task 340: 133 vs 119 bytes for gold, https://arcprize.org/play?task=d687bc17
p=lambda i,k=3:-k*i or[[x[0],sum({*x[1:]}&{*x[:2]}),*[y*(sum(i,i).count(y)>4>0<y!=x[0])for y in x[2:]]]for x in zip(*p(i,k-1)[::-1])]
# task 341: 138 vs 132 bytes for gold, https://arcprize.org/play?task=d6ad076f
...
# task 354: 100 vs 96 bytes for gold, https://arcprize.org/play?task=ddf7fa4f
...
# task 357: 91 vs 86 bytes for gold, https://arcprize.org/play?task=e179c5f4
...
# task 361: 195 (245 unzipped) bytes, gold, https://arcprize.org/play?task=e40b9e2f
...
# task 363: 234 vs 213 bytes for gold, https://arcprize.org/play?task=e5062a87
E=enumerate
def p(g):
...
# task 367: 222 vs 130 bytes for gold, https://arcprize.org/play?task=e73095fd
...
# task 370: 269 (357 unzipped) bytes, gold, https://arcprize.org/play?task=e8dc4411
def p(g,k=3):
...
p=lambda a:[*zip(*a)][::-1]
# task 381: 92 vs 84 bytes for gold, https://arcprize.org/play?task=ef135b50
import re
...
# task 390: 122 vs 99 bytes for gold, https://arcprize.org/play?task=f8a8fe49
p=lambda g,k=-1:k*g or p([*zip(*[[*[g[k-~k]for k in(1,2,-3,-4)if g[k].count(2)>4],r][0]for r in g if(g:=g[1:]+[r])])],k+1)
# task 391: 63 bytes, gold, https://arcprize.org/play?task=f8b3ba0a
p=lambda m:[*zip(sorted({*(a:=sum(m,[]))},key=a.count))][2::-1]
# task 392: 178 vs 156 bytes for gold, https://arcprize.org/play?task=f8c80d96
def p(g):e=enumerate;C=max(max(g));[g:=[[c or _*any(any(s[x+x%~x:x+2])for s in g[y+y%~y:y+2])for x,c in e(r)]for y,r in e(g)]for _ in[5,5,C][f"{C}, 0, {C}"in str(g):]*3];return g
...
# task 396: 188 bytes, gold, https://arcprize.org/play?task=fcb5c309
E=enumerate
