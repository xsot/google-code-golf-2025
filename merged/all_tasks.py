...
# task 5: 226 (244 unzipped) vs 206 bytes for gold, https://arcprize.org/play?task=045e512c
W=range(21)
t=-4,0,4
...
# task 18: 332 (374 unzipped) bytes, gold, https://arcprize.org/play?task=0e206a2e
E=enumerate
def p(g):
...
# task 34: 181 vs 148 bytes for gold, https://arcprize.org/play?task=1f0c79e5
...
# task 35: 87 bytes, gold, https://arcprize.org/play?task=1f642eb9
...
# task 44: 283 (360 unzipped) bytes, gold, https://arcprize.org/play?task=228f6490
def p(i,r=range(10)):
 for n in{*sum(i,[])}:
...
# task 48: 136 vs 95 bytes for gold, https://arcprize.org/play?task=239be575
...
p=lambda a:(a+a)[2:5]
# task 54: 381 (589 unzipped) vs 356 bytes for gold, https://arcprize.org/play?task=264363fd
def p(g):
...
# task 66: 351 (479 unzipped) vs 299 bytes for gold, https://arcprize.org/play?task=2dd70a9a
def p(g):
...
# task 71: 235 (247 unzipped) vs 122 bytes for gold, https://arcprize.org/play?task=3345333e
...
# task 76: 306 (355 unzipped) bytes, gold, https://arcprize.org/play?task=36d67576
E=enumerate
def p(g):
...
# task 80: 313 (465 unzipped) vs 283 bytes for gold, https://arcprize.org/play?task=39e1d7f9
def p(g):
...
# task 85: 56 bytes, gold, https://arcprize.org/play?task=3bdb4ada
...
# task 89: 292 (376 unzipped) vs 289 bytes for gold, https://arcprize.org/play?task=3e980e27
def p(g):
 B=[];e=enumerate
 for A,r in e(g):
  for x,F in e(r):
...
# task 101: 335 (447 unzipped) bytes, gold, https://arcprize.org/play?task=447fd412
...
# task 118: 333 (412 unzipped) bytes, gold, https://arcprize.org/play?task=50846271
def p(i,n=2,R=range):
...
# task 170: 243 (279 unzipped) vs 211 bytes for gold, https://arcprize.org/play?task=6ecd11f4
def p(g):
...
p=lambda a:a+a[::-1]
# task 173: 248 (277 unzipped) vs 247 bytes for gold, https://arcprize.org/play?task=72322fa7
def p(g):
...
# task 201: 255 (306 unzipped) vs 217 bytes for gold, https://arcprize.org/play?task=846bdb03
def p(g):
...
# task 205: 225 (235 unzipped) vs 189 bytes for gold, https://arcprize.org/play?task=8731374e
...
# task 208: 258 (290 unzipped) vs 247 bytes for gold, https://arcprize.org/play?task=890034e9
def p(g):
 e=enumerate;A=g
...
# task 209: 299 (406 unzipped) bytes, gold, https://arcprize.org/play?task=8a004b2b
def p(g):
 R=range;G=g[-5:]
...
# task 233: 329 (460 unzipped) bytes, gold, https://arcprize.org/play?task=97a05b5b
def p(g):
...
# task 238: 234 (247 unzipped) vs 228 bytes for gold, https://arcprize.org/play?task=9aec4887
...
# task 255: 274 (313 unzipped) bytes, gold, https://arcprize.org/play?task=a64e4611
def p(g):
...
# task 268: 286 (327 unzipped) vs 250 bytes for gold, https://arcprize.org/play?task=aba27056
def p(g,k=3,v=range):
...
# task 270: 119 bytes, gold, https://arcprize.org/play?task=ae3edfdc
...
# task 308: 269 (316 unzipped) vs 254 bytes for gold, https://arcprize.org/play?task=c8cbb738
def p(g):
...
# task 319: 306 (392 unzipped) vs 199 bytes for gold, https://arcprize.org/play?task=ce602527
E=enumerate
def p(g):
...
# task 324: 291 (343 unzipped) bytes, gold, https://arcprize.org/play?task=d07ae81c
def p(g,e=enumerate):
 ...
 return g
# task 325: 184 vs 161 bytes for gold, https://arcprize.org/play?task=d0f5fe59
...
# task 361: 220 (235 unzipped) vs 204 bytes for gold, https://arcprize.org/play?task=e40b9e2f
...
# task 363: 243 (300 unzipped) vs 217 bytes for gold, https://arcprize.org/play?task=e5062a87
def p(g):
...
# task 368: 138 vs 142 bytes for gold, https://arcprize.org/play?task=e76a88a6
import re
...
# task 370: 277 (342 unzipped) bytes, gold, https://arcprize.org/play?task=e8dc4411
def p(g,k=3):
...
# task 371: 114 vs 115 bytes for gold, https://arcprize.org/play?task=e9614598
def p(a):
...
# task 378: 238 vs 145 bytes for gold, https://arcprize.org/play?task=ec883f72
def p(g):
...
p=lambda a:[*zip(*a)][::-1]
# task 381: 94 vs 87 bytes for gold, https://arcprize.org/play?task=ef135b50
import re
...
# task 390: 126 vs 103 bytes for gold, https://arcprize.org/play?task=f8a8fe49
...
# task 392: 203 vs 161 bytes for gold, https://arcprize.org/play?task=f8c80d96
def p(g):
...
# task 394: 126 vs 91 bytes for gold, https://arcprize.org/play?task=f9012d9b
p=lambda i,n=1:all(len({*x[::n],0})<3for x in i)*[[max(x[b%n::n])for b,y in enumerate(x)if y<1]for x in i if 0in x]or p(i,n+1)
# task 395: 55 vs 53 bytes for gold, https://arcprize.org/play?task=fafffa47
p=lambda a:[[2&~c+b.pop(0)for c in a.pop(3)]for b in a]
# task 396: 261 (268 unzipped) vs 220 bytes for gold, https://arcprize.org/play?task=fcb5c309
def p(m):
...
# task 400: 70 vs 6 bytes for gold, https://arcprize.org/play?task=ff805c23
