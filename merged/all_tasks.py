...
# 💎 task 44: 235 (350 unzipped) bytes, gold, https://arcprize.org/play?task=228f6490
def p(i):
 for n in range(10):
  for a in range(10):
   for b in range(10):
...
# 💎 task 46: 169 bytes, gold, https://arcprize.org/play?task=234bbc79
...
# 🥉 task 69: 159 vs 151 bytes for gold, https://arcprize.org/play?task=321b1fc6
...
# ❌ task 154: 110 vs 99 bytes for gold, https://arcprize.org/play?task=6855a6e4
...
# 💎 task 205: 164 bytes, gold, https://arcprize.org/play?task=8731374e
...
# ❌ task 213: 94 vs 89 bytes for gold, https://arcprize.org/play?task=8e1813be
...
def p(g):
 ...
 for s in [[v[x:x+3]for v in G[y:y+3]]for y,p in e(G[2:])for x,p in e(p[2:])][::-1]*4:
  ...
  s[:]=zip(*s[::-1])
 return g
# 🥈 task 234: 116 vs 115 bytes for gold, https://arcprize.org/play?task=98cf29f8
...
# 💎 task 245: 101 bytes, gold, https://arcprize.org/play?task=a1570a43
...
# 🥉 task 324: 260 (313 unzipped) vs 245 bytes for gold, https://arcprize.org/play?task=d07ae81c
...
# ❌ task 341: 136 vs 132 bytes for gold, https://arcprize.org/play?task=d6ad076f
p=lambda i:[i:=[[i[b][a]or all(len({*s[b:]})%len({*s})&2for s in[*zip(*i)][a+a%~a:a+2])*8for b in r]for a in r]for r in[range(10)]*2][1]
# 🥈 task 342: 112 vs 111 bytes for gold, https://arcprize.org/play?task=d89b689b
...
# 🥉 task 349: 224 (274 unzipped) vs 214 bytes for gold, https://arcprize.org/play?task=db93a21d
...
# 💎 task 370: 242 (1319 unzipped) bytes, gold, https://arcprize.org/play?task=e8dc4411
def p(g):
 N=[*map(min,g)].count(0)
 x=[*map(min,g)].index(0)
...
# 🥉 task 387: 221 vs 218 bytes for gold, https://arcprize.org/play?task=f35d900a
...
# ❌ task 390: 111 vs 99 bytes for gold, https://arcprize.org/play?task=f8a8fe49
