...
def p(g):G={i*1j+j:g for i,g in enumerate(g)for j,g in enumerate(g)if g};[(s:={J},[s:={J}|s for J in[*G]*#[*range(5,10)]##for x in s if abs(J-x)<2],[*s][3:]and[#[*range(10)]##for a in[1,3,6,7]for O in G if all(sum(G[x]==G.get(j)for j in[*s,(x-J-a//#[4,5]##*(x-J).real*2)*1j**a+O])>1for x in s)for x in s if(g:=[[{x:0,(x-J-a//#[prev_vals[-1]]##*(x-J).real*2)*1j**a+O:G[x]}.get(i*1j+j,g)for j,g in enumerate(g)]for i,g in enumerate(g)])])for J in G];return g
