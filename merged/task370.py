# joking (234 (367 unzipped) bytes, gold)
def p(g):
 for s in range(8):
...
def p(g):
 for s in range(#[4,8]##):
  #[(t1:='x=[*map(min,g)].index(0);')+(t2:='N=[*map(min,g)].count(0);'),t2+t1]##g=[*map(list,zip(*g))][::-1];y=[*map(min,g)].index(0)
  for s in range(#[4,8]##):
   for i in range(N):
    for j in range(N):
...
### combined (274 (342 unzipped) bytes)
def p(g,k=3):
